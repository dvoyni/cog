...
// is checked against.
const VertexTypeCount = gpu.Unorm1010102 + 1
...
func Attr(offset int, typ gpu.VertexType) VertexAttr { return VertexAttr{offset: offset, typ: typ} }
...
func VertexLayoutKeyOf(layout []VertexAttr) (VertexLayoutKey, bool) {
	...
	for i := range layout {
		...
		if attr.offset < 0 || attr.offset+size > MaxVertexStride ||
			attr.typ <= gpu.UnknownVertexType || attr.typ >= VertexTypeCount {
...
func Mesh(vertices BufferDescr, topology gpu.PrimitiveTopology, layout ...VertexAttr) MeshDescr {
	return MeshIndexed(vertices, BufferDescr{}, gpu.IndexUint32, topology, layout...)
}
...
func (m MeshDescr) IndexWidth() gpu.IndexWidth { return m.indexWidth }
...
func (m MeshDescr) Topology() gpu.PrimitiveTopology { return m.topology }
