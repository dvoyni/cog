...
// The name tables of the recorder-side enums. They live here rather than in the
// contract root on purpose: naming an enum for a debug document is not the same
// promise as giving every gfx enum a String. The root's views and gfximpl's
// frame snapshot both read them; the GPU vocabulary's enums name themselves
// through gpu's Name methods. An unknown value names its ordinal rather than
