...
func TestPreparedParameterPlanReusesShapeAndReadsCurrentValues(t *testing.T) {
	...
	material = []gfx.ParameterDescr{gfx.TextureParam("texture", internal.BakedTexture(2, 0, 0))}
	...
	if got := internal.ParameterNum(second.uniforms[0].param.value(material, draw)); got != 9 {
		t.Fatalf("prepared uniform value = %v, want current value 9", got)
	}
	if got := internal.ParameterTexture(second.resources[0].param.value(material, draw)).ID(); got != 2 {
...
func TestPreparedParameterPlanKeysOrderAndPreservesFirstDrawMatch(t *testing.T) {
	...
	if got := internal.ParameterNum(plan.uniforms[0].param.value(material, draw)); got != 2 {
