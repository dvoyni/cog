...
type flushPlugin struct {
	mu     sync.Mutex
	record func(*gfx.OpQueue)
}

type flushUpdateHandler kernel.Subscription[app.UpdateEvent]

func (*flushPlugin) Name() kernel.PluginName           { return "gfxtestflush" }
func (*flushPlugin) Dependencies() []kernel.PluginName { return []kernel.PluginName{gfx.Name} }

func (f *flushPlugin) Register(r *kernel.Registrar, _ any) error {
	r.Subscribe[flushUpdateHandler](f.flushOnUpdate).Last().Before[gfx.PresentOnUpdate]()
	return nil
}
...
func (f *flushPlugin) on(record func(*gfx.OpQueue)) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.record = record
}
...
func (r *captureRig) runSnapshot(request gfx.FrameRequest, record func(*gfx.OpQueue)) (gfx.FrameResponse, error) {
	r.t.Helper()
	type answer struct {
		response gfx.FrameResponse
		err      error
	}
...
func TestAFrameSnapshotReportsPassesInRunOrderWithTheirCounts(t *testing.T) {
	...
	response, err := rig.runSnapshot(gfx.FrameRequest{}, twoPasses)
	if err != nil {
...
func TestAFilteredSnapshotKeepsSourceIndicesAndNamesWhatItDropped(t *testing.T) {
	...
	whole, err := rig.runSnapshot(gfx.FrameRequest{}, twoPasses)
	if err != nil {
		t.Fatalf("unfiltered snapshot: %v", err)
	}
	filtered, err := rig.runSnapshot(gfx.FrameRequest{Pass: "world"}, twoPasses)
	if err != nil {
...
func TestAFrameSnapshotReportsResourceTrafficFromBothQueues(t *testing.T) {
	...
	response, err := rig.runSnapshot(gfx.FrameRequest{}, func(q *gfx.OpQueue) {
	...
	byKind := map[string][]gfx.ResourceOpView{}
...
func TestAFrameSnapshotBindsToATickThatBeganAfterTheRequest(t *testing.T) {
	...
	before.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthAuto(), Label: "before"})
	drawInto(before)
	...
	after.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthAuto(), Label: "after"})
	drawInto(after)
	rig.tick()
...
func TestASecondFrameSnapshotIsRefusedInWordsWhileOneIsInFlight(t *testing.T) {
	...
	_, err := frameSnapshot(rig.k, gfx.FrameRequest{})
...
func TestAFrameSnapshotUnderPausePerformsOneStepAndSaysSo(t *testing.T) {
	...
	response, err := frameSnapshot(rig.k, gfx.FrameRequest{})
	if err != nil {
		t.Fatalf("snapshot: %v", err)
	}
	if !response.Stepped {
...
func TestAFrameSnapshotIsWrittenToThePathTheAgentNames(t *testing.T) {
	...
	response, err := rig.runSnapshot(gfx.FrameRequest{Path: path}, twoPasses)
	if err != nil {
		t.Fatalf("snapshot: %v", err)
	}
...
func TestAFrameSnapshotRefusesAPathItCannotHonour(t *testing.T) {
	...
	for _, test := range []struct{ name, path, wants string }{
		...
		t.Run(test.name, func(t *testing.T) {
			_, err := frameSnapshot(rig.k, gfx.FrameRequest{Path: test.path})
			var refusal mcp.Unavailable
			if !errors.As(err, &refusal) {
...
func TestAFrameSnapshotSeesWhatALateRecorderFlushedIntoTheQueue(t *testing.T) {
	...
	rig.flush.on(func(q *gfx.OpQueue) {
		q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthAuto(), Order: 100, Label: "canvas"})
		drawInto(q)
	})
...
func TestTheNewAccessorsAnswerOutsideTheAgentPath(t *testing.T) {
	...
	// merely unlikely.
	matrix := gfx.MatParam("mvp", m.NewMat4())
