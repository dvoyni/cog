...
func TestSamplingWhatAnEarlierPassRenderedGetsABarrier(t *testing.T) {
	...
	want := gpu.TextureTransition{Texture: sampled, From: gpu.TextureUsageRenderAttachment, To: gpu.TextureUsageTextureBinding}
...
func TestAPassThatSamplesNothingItRenderedGetsNoBarrier(t *testing.T) {
	...
	backend := transitionFrame(t, func(q *gfx.OpQueue) {
		target, _ := q.TemporaryTarget(64, 64, gpu.FormatRGBA8Srgb)
		q.Pass(gfx.PassDescr{Target: target, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 0, Label: "offscreen"})
		drawInto(q)
		q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 1, Label: "screen"})
		drawInto(q)
	})
...
func TestRenderingIntoATextureAnEarlierPassSampledGetsTheReverseBarrier(t *testing.T) {
	...
	var pong gpu.TextureID
	backend := transitionFrame(t, func(q *gfx.OpQueue) {
		targetA, textureA := q.TemporaryTarget(64, 64, gpu.FormatRGBA8Srgb)
		targetB, textureB := q.TemporaryTarget(32, 32, gpu.FormatRGBA8Srgb)
		pong = textureB.ID()

		// B is written, then read, then written again.
		q.Pass(gfx.PassDescr{Target: targetB, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 0, Label: "b-first"})
		drawInto(q)
		q.Pass(gfx.PassDescr{Target: targetA, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 1, Label: "a-reads-b"})
		q.Draw(triangle(), testMaterial(gfx.TextureParam("MainTexture", textureB)), gfx.MatParam("mvp", m.NewMat4()))
		q.Pass(gfx.PassDescr{Target: targetB, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 2, Label: "b-again"})
		q.Draw(triangle(), testMaterial(gfx.TextureParam("MainTexture", textureA)), gfx.MatParam("mvp", m.NewMat4()))
	})

	forward := gpu.TextureTransition{Texture: pong, From: gpu.TextureUsageRenderAttachment, To: gpu.TextureUsageTextureBinding}
	...
	reverse := gpu.TextureTransition{Texture: pong, From: gpu.TextureUsageTextureBinding, To: gpu.TextureUsageRenderAttachment}
...
func TestADepthAttachmentSampledLaterGetsABarrier(t *testing.T) {
	...
	drawInto(q)
	q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 1, Label: "lit"})
	q.Draw(triangle(), testMaterial(gfx.TextureParam("MainTexture", shadow)), gfx.MatParam("mvp", m.NewMat4()))
	k.ExecuteCommand[gfx.PresentCmd](gfx.PresentRequest{})
	k.PublishEvent(app.RenderEvent{}).Wait()

	want := gpu.TextureTransition{Texture: shadow.ID(), From: gpu.TextureUsageRenderAttachment, To: gpu.TextureUsageTextureBinding}
...
func TestATextureStaysTransitionedAcrossConsecutivePassesThatSampleIt(t *testing.T) {
	...
	backend := transitionFrame(t, func(q *gfx.OpQueue) {
		source, texture := q.TemporaryTarget(64, 64, gpu.FormatRGBA8Srgb)
		other, _ := q.TemporaryTarget(32, 32, gpu.FormatRGBA8Srgb)
		q.Pass(gfx.PassDescr{Target: source, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 0, Label: "offscreen"})
		drawInto(q)
		q.Pass(gfx.PassDescr{Target: other, Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 1, Label: "reads-once"})
		q.Draw(triangle(), testMaterial(gfx.TextureParam("MainTexture", texture)), gfx.MatParam("mvp", m.NewMat4()))
		q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthNone(), Load: gpu.LoadClear, Order: 2, Label: "reads-again"})
		q.Draw(triangle(), testMaterial(gfx.TextureParam("MainTexture", texture)), gfx.MatParam("mvp", m.NewMat4()))
	})
...
func TestNoPassIsHandedAnEmptyBarrierList(t *testing.T) {
	...
	backend := transitionFrame(t, func(q *gfx.OpQueue) {
		q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthNone(), Load: gpu.LoadClear, Label: "screen"})
		drawInto(q)
	})
...
func TestAnOrdinaryTextureIsNeverTransitioned(t *testing.T) {
	...
	withResourceQueue(t, k, func(resources *gfx.ResourceQueue) {
		uploaded = resources.AllocateTexture(8, 8, 1, gpu.FormatRGBA8Srgb)
	})
	q := recordRaw(t, k)
	q.Pass(gfx.PassDescr{Target: gfx.ScreenTarget(), Depth: gfx.DepthNone(), Load: gpu.LoadClear, Label: "screen"})
