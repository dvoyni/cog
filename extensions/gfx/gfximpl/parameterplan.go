...
func (d declaredKind) accepts(kind internal.ParamKind) bool {
	switch d {
	case declaredTexture:
		return kind == internal.ParamTexture
	case declaredSampler:
		return kind == internal.ParamSampler
	case declaredBuffer:
		return kind == internal.ParamBuffer
	}
...
func (plan *parameterPlan) checkKind(label, name string, ref parameterRef, material, draw []gfx.ParameterDescr, declared declaredKind) {
	...
	if param == nil || declared.accepts(internal.ParameterKind(param)) {
		return
	}
	plan.mismatch = gfx.ErrParameterKindMismatch{
		Shader: label, Parameter: name, Supplied: internal.ParameterKind(param).String(), Declared: declared.String(),
	}
}
