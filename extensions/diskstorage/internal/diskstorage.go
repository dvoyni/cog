...
func openDiskFS(path string) (storage.PermanentFS, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("diskstorage: resolve permanent directory %q: %w", path, err)
	}
	if err := os.MkdirAll(absolute, 0o700); err != nil {
		return nil, fmt.Errorf("diskstorage: create permanent directory %q: %w", absolute, err)
	}
	root, err := os.OpenRoot(absolute)
	if err != nil {
		return nil, fmt.Errorf("diskstorage: open permanent directory %q: %w", absolute, err)
	}
	if err := root.Close(); err != nil {
		return nil, fmt.Errorf("diskstorage: close permanent directory %q: %w", absolute, err)
	}
