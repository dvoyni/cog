...
// nosound reads the Loop Region out of the same header pass that already gives
// it the duration, the channels and the rate, and reaches the same answer
// otosound reaches over the same bytes. That parity is the whole of what this
// Adapter is for: a test that passes here says something about the same game
// under a real device, and a loop point a silent Adapter could not see would
// make a Clip mean two things.
//
...
// and reads the three tags case-insensitively.
func TestNosoundReportsTheLoopRegionFromTheHeaderPass(t *testing.T) {
	cases := []struct {
		name       string
		tags       []string
		start, end int64
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			prepared, _, err := newBackend(48000).Prepare(nil, retagged(t, fixture(t), c.tags...))
			if err != nil {
...
// A malformed region is dropped whole and never clamped, and the Adapter says
// so once per Clip - through the one subscription nosound has, because a
// Backend holds no Kernel of its own. A game's suite learns its music file's
// tags are broken without a device in the room.
func TestNosoundDropsAMalformedRegionWholeAndReportsItOncePerClip(t *testing.T) {
	b := newBackend(48000)

	prepared, _, err := b.Prepare(nil, retagged(t, fixture(t), "LOOPSTART=11025", "LOOPEND=48705"))
	if err != nil {
		t.Fatalf("a malformed loop tag failed the Clip: %v", err)
	}
	if prepared.LoopRegion().Present() {
		t.Fatal("a region ending past the Clip was kept, and it must be dropped whole")
	}
	if prepared.Duration() != clipDuration {
		t.Fatalf("the Clip reports %v, want the file's %v", prepared.Duration(), clipDuration)
	}
...
func TestNosoundSaysNothingAboutAnUntaggedClip(t *testing.T) {
	b := newBackend(48000)

	prepared, _, err := b.Prepare(nil, fixture(t))
	if err != nil {
