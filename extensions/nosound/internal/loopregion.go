...
// otosound runs in clipBounds, over the same bytes, reaching the same answer.
//
...
// says nothing about the game under otosound. The duration is already written
...
// a LOOPEND is only in range against that same count. Resolved apart, the
// region would be checked against a length the Clip does not have.
//
