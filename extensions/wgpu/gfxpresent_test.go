...
func TestThePresentShaderTakesItsTransferFunctionFromTheFrameBufferFormat(t *testing.T) {
	...
	if !strings.Contains(presentSource(gpu.FormatRGBA8Srgb), "1.0 / 2.4") {
	...
	if strings.Contains(presentSource(gpu.FormatRGBA8), "1.0 / 2.4") {
	...
	if presentSource(gpu.FormatScreen) != presentSource(gpu.FrameBufferFormat) {
...
func TestBothPresentShadersCompile(t *testing.T) {
	...
	for _, format := range []gpu.TextureFormat{gpu.FormatRGBA8, gpu.FormatRGBA8Srgb} {
...
func TestThePresentShaderDrawsOneFullScreenTriangle(t *testing.T) {
	source := presentSource(gpu.FrameBufferFormat)
