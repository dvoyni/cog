...
// plugin is the wgpu driver. Its configuration arrives through the configured map
// under wgpu.Name.
type plugin struct {
	config cwgpu.Config
	gpu    *gogpu.App
...
// New creates the wgpu plugin. Its wgpu.Config is supplied at Register through
...
func (p *plugin) Register(registrar *kernel.Registrar, config any) error {
	...
	if config != nil {
		...
		if !ok {
			return cwgpu.ErrInvalidConfig{Got: config}
		}
		cfg = c
	}
