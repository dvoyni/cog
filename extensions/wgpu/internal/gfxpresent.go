...
func presentSource(format gfx.TextureFormat) string {
	const header = `
...
func (b *gfxBackend) frameBuffer() *gfxbTexture {
	...
	frame, err := b.newTexture(gfx.TextureDesc{
		Width: b.screenW, Height: b.screenH, Layers: 1,
		Format: gfx.FrameBufferFormat, Renderable: true, Label: "gfx.framebuffer",
	})
	if err != nil {
		return nil
	}
...
func (b *gfxBackend) presentPipeline() *wgpu.RenderPipeline {
	...
	module, err := b.device.CreateShaderModule(&wgpu.ShaderModuleDescriptor{
		Label: "gfx.present", WGSL: presentSource(gfx.FrameBufferFormat),
	})
	if err != nil {
		return nil
	}
