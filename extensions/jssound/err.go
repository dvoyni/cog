...
func (e ErrInvalidConfig) Error() string {
	return fmt.Sprintf("jssound: invalid config: want %T, got %T", Config{}, e.Got)
}

// ErrInvalidLatencyHint reports a negative latency hint. Zero is interactive,
// the default; a negative latency is not a latency.
type ErrInvalidLatencyHint struct{ LatencyHint time.Duration }

func (e ErrInvalidLatencyHint) Error() string {
...
func (e ErrDeviceUnavailable) Error() string {
	return fmt.Sprintf("jssound: no Web Audio context could be opened: %v", e.Err)
}
...
func (e ErrNotOggVorbis) Error() string {
	return fmt.Sprintf("jssound: not readable as ogg vorbis: %v", e.Err)
}
...
// without a codec.
type ErrDecodeRefused struct{ Message string }

func (e ErrDecodeRefused) Error() string {
...
func (e ErrNoStreamFormat) Error() string {
	return fmt.Sprintf("jssound: the stream reports %d Hz and %d channels", e.SampleRate, e.Channels)
}
...
func (e ErrLoopRegionIgnored) Error() string {
	if e.Err != nil {
		return fmt.Sprintf(
			"jssound: a Clip's loop tags could not be read, so it loops whole: %v", e.Err)
	}
	return fmt.Sprintf(
		"jssound: a Clip's loop region [%d, %d) frames is not inside its %d frames, so it loops whole",
