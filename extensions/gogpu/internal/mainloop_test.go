...
// The gogpu Extension is app's MainLoop: each gogpu library update hands the
// Loop app attached the real time of the frames drawn since the last one, after
// flushing the frame's input, so every tick the Loop publishes sees that input.
// The window itself is not needed to show it: onUpdate runs against a bare
// plugin and a real engine.
...
func (*mainLoopHarness) Name() kernel.PluginName           { return "gogpumainlooptest" }
