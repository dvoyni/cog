...
// than importing the scene package. gogpu is the only tree with a WGSL front
// end, so the check has to live here, and reading the sources keeps the driver
// from depending on a plugin that sits above it.
//
...
func bundledSceneShader(t *testing.T, opts ...cgfx.ShaderOption) string {
	t.Helper()
	text, err := flattenShader(t, "scene", os.DirFS("../../../bundles/model/internal"), cgfx.ShaderWithResource("builtin/model/scene.wgsl", opts...))
	if err != nil {
...
// against the same members, which is the half gogpu cannot reach from here.
