...
// bundledShader flattens one of the engine's own root shaders off disk. gogpu
// is the only tree with a WGSL front end, so the reflection half of the check
...
func bundledShader(t *testing.T, mount storage.MountId, dir, path string, opts ...cgfx.ShaderOption) string {
	t.Helper()
	text, err := flattenShader(t, mount, os.DirFS(dir), cgfx.ShaderWithResource(path, opts...))
	if err != nil {
...
// the two it adds are spelled here, at the offsets bundles/model/internal/vertexpack.go names.
//
...
func canvasShader(t *testing.T, path string) string {
	t.Helper()
	return bundledShader(t, "canvas", "../../../bundles/canvas/internal", path)
}
