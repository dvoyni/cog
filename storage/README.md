...
file. Saved data reads back through `ReadFS` like any other asset.
...
scope. Mount management goes through the commands below, since `ReadFS`'s mount
list can only be mutated that way; reading and writing files is ordinary direct
resource access: a handler declares `access.GetRead[storage.ReadFS]()` or
`access.GetWrite[storage.WriteFS]()` and calls `fs.ReadFile`/`WriteFile`/
`MkdirAll`/`Remove`/`Rename` on the resource itself.
