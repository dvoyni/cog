...
[slots/gfx/docs/specs/capture.md](./capture.md). This document specifies only what the
...
> **Amended by [#366](https://github.com/dvoyni/cog/issues/366).** gfx moved
> to `slots/gfx` in the declaration-root shape of
> [ADR 0002](../../../../docs/adr/0002-slots-extensions-and-bundles-as-declaration-roots.md). The Provider, the two capability bodies and
> their request and response types moved with the plugin from `gfximpl` to
> `slots/gfx/internal/mcpprovider.go`, and `frameOnUpdate` stays unexported
> there. `ArmCaptureCmd` and `ArmFrameCmd` with their requests and responses are
> in the root's `commands.go`; `FrameSnapshot`, `FrameView`, `PassView` and
> `ResourceOpView` in its `types.go`; the shared view types are declared in
> `slots/gfx/internal/types/views.go` and aliased in `types.go`, their `…ViewOf`
> constructors forwarded from `utils.go`. `McpProvider` stays in `adapters.go`.
> The plugin is constructed with `gfxplugin.New()`. Tool names, schemas and
...
[mcp §Annotations](../../../../bundles/mcp/docs/specs/mcp.md#annotations) for why that is
not a lie.

Both follow
[mcp §Arm-then-wait](../../../../bundles/mcp/docs/specs/mcp.md#arm-then-wait) and neither
restates it.
...
([mcp §Delivery](../../../../bundles/mcp/docs/specs/mcp.md#delivery-path)): absolute only,
...
- **The declared passes**, in run order, each with `Label`, `Order`, target,
  ...
  (`extensions/gfx/pass.go:135-146`), and each carrying its declaration index.
...
- **The resource ops in full** — `opBakeBuffer`, `opBakeTexture`,
  ...
  `opFreeCachedResources` and the rest (`extensions/gfx/opqueue.go:11-53`) — each with its
  ...
  > `op.pass` is set by `draw` and by nothing else (`extensions/gfx/opqueue.go`), and
...
(`extensions/gfx/parameter.go:13-31`), `TextureDescr` (`extensions/gfx/texture.go:6-15`),
`MaterialDescr` (`extensions/gfx/material.go:13-17`), `MeshDescr` (`extensions/gfx/mesh.go:102-110`) —
...
4. **Nothing checks the mirror.** Reorder a field and it silently reinterprets a
   ...
   checked by nobody. (`bundles/canvas/inspect.go:builtinVertices` uses `unsafe`
...
`TextureValue`/`SamplerValue`/`VecValue`/`HasValue` (`extensions/gfx/parameter.go:128-149`)
each return `(value, ok)` keyed on the private `kind`; `TextureDescr.ID`/`Path`/
`Size` (`extensions/gfx/texture.go:18-25`); `MaterialDescr.State`/`Fingerprint`
(`extensions/gfx/material.go:70,87`). Roughly **eight one-line methods** complete it —
...
[mcp §Under pause](../../../../bundles/mcp/docs/specs/mcp.md#under-pause).
...
**`extensions/gfx/snapshot.go`** (new)
...
**`extensions/gfx/parameter.go`, `extensions/gfx/texture.go`, `extensions/gfx/material.go`, `extensions/gfx/mesh.go`**
