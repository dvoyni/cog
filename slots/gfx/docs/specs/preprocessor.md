...
`github.com/dvoyni/cog/extensions/gfx` resolves a shader's text before it reaches
...
> [ADR 0002](../../../../docs/adr/0002-slots-extensions-and-bundles-as-declaration-roots.md), and `FlattenShader` left its API: its
> signature names `storage.FileSystem`, which a Slot's forwarders may not. The
> preprocessor is in `slots/gfx/internal/types` (`directive.go`,
> `condition.go`, `flatten.go`, `hoist.go`), its own tests beside it, and the
> translator in `slots/gfx/internal` calls it on a cache miss as before. Tests
> outside gfx that need a flattened module — canvas's, scene's and wgpu's —
> draw once with the shader through `gfxplugin.New()` and a recording backend,
...
- **Module-scope order is not significant** (measured against `naga.Parse` +
  `wgsl.Lower` from `github.com/gogpu/naga`, the pair `extensions/wgpu/gfxreflect.go`
...
`materialKey` re-hashes the key **per draw per frame** (`extensions/gfx/material.go:87`),
...
input, in writing: `DefaultLimits` (`extensions/gfx/contract.go`) is the comparison target
...
(`extensions/gfx/translate.go:511`) evicts every module whose set contains the released
path.
...
`extensions/gfx/commands.go:25`), `t.shaders` holds single digits, and the forward
...
`Backend.NewShader` and `checkWebLimits` (`extensions/gfx/translate.go:505`), and it is
...
- **The pure-Go backend swallows WGSL compile errors.**
  ...
  `extensions/wgpu/gfxbackend.go`. So these diagnostics reach a developer only through the
...
`wgsl.Lower`, the same path `extensions/wgpu/gfxreflect.go` reflects with.
...
duplicated: `bundles/scene/light.go:65` declares `const maxLights = 16`, the shader
independently spells `array<SceneLight, 16>`, and `bundles/scene/light_test.go` pins the
...
`prepareParameterPlan` (`extensions/gfx/parameterplan.go:72`) walks the **reflected layout**
and looks up a parameter by name for each resource, so a parameter that is
supplied but no longer declared is **never visited**. `bundles/scene/draw.go` can keep
...
draw had to bind group 2 (`bundles/scene/lookup.go`); under the split a draw with no
...
(`bundles/scene/err.go`) guards only the other way round and states outright that "the
