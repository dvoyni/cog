...
func TestViewportFixedWidth(t *testing.T) {
	...
	k.ExecuteCommand[SetDesiredViewportCmd](
		SetDesiredViewportRequest{Mode: types.ViewportFixedWidth, Size: 1280})

	response := k.ExecuteCommand[SetViewportCmd](SetViewportRequest{Width: 1920, Height: 1080})
	want := types.Viewport{Width: 1280, Height: 720, WindowWidth: 1920, WindowHeight: 1080}
	...
	want = types.Viewport{Width: 1280, Height: 2560, WindowWidth: 500, WindowHeight: 1000}
	if response.Viewport != want {
...
func TestViewportCarriesFramebufferSizeAcrossPreferenceChanges(t *testing.T) {
	...
	desiredResponse := k.ExecuteCommand[SetDesiredViewportCmd](SetDesiredViewportRequest{
		Mode: types.ViewportFit, Width: 1280, Height: 720,
	})
...
func TestViewportFixedHeight(t *testing.T) {
	k := newTestKernel(t, newPlugin())
	k.ExecuteCommand[SetDesiredViewportCmd](
		SetDesiredViewportRequest{Mode: types.ViewportFixedHeight, Size: 720})

	response := k.ExecuteCommand[SetViewportCmd](SetViewportRequest{Width: 1000, Height: 1000})
	want := types.Viewport{Width: 720, Height: 720, WindowWidth: 1000, WindowHeight: 1000}
	if response.Viewport != want {
		t.Errorf("viewport = %#v, want %#v", response.Viewport, want)
	}
}
...
func TestViewportFitAndCover(t *testing.T) {
	tests := []struct {
		...
		{name: "fit wider", mode: types.ViewportFit, width: 2000, height: 1000, want: types.Viewport{Width: 1440, Height: 720, WindowWidth: 2000, WindowHeight: 1000}},
		{name: "fit narrower", mode: types.ViewportFit, width: 1000, height: 1000, want: types.Viewport{Width: 1280, Height: 1280, WindowWidth: 1000, WindowHeight: 1000}},
		{name: "cover wider", mode: types.ViewportCover, width: 2000, height: 1000, want: types.Viewport{Width: 1280, Height: 640, WindowWidth: 2000, WindowHeight: 1000}},
		{name: "cover narrower", mode: types.ViewportCover, width: 1000, height: 1000, want: types.Viewport{Width: 720, Height: 720, WindowWidth: 1000, WindowHeight: 1000}},
	}
