...
type Loop interface {
	// Init publishes InitEvent. The MainLoop calls it once, on the thread that
	...
	// Frame runs one frame of the fixed-step update on the MainLoop's main
	...
	// and only requested steps publish. A MainLoop flushes the frame's input
	...
	// interpolation factor the last Frame left. The MainLoop calls it on its
	// render thread once the frame's target is current; it may run while Frame
	// runs on the main thread.
	Render(k kernel.Executioner)
	// WindowSize publishes WindowSizeChangeEvent. The MainLoop calls it when the
	...
	// Quit publishes QuitEvent. The MainLoop calls it once, after its loop
	// returns.
