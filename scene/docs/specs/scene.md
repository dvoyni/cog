...
pass. It is in the pinned version and in the newest published one. `cog/wgpu`
...
draw. Until the HAL gap closes, a depth-only pass may be *written* but its output
may not be *depended on* in the same frame, which is exactly the constraint
...
- `GpuPassDesc` could not distinguish `NoTarget()` from a texture target whose
  ...
  **every temporary target is unresolved on its first frame**, since its
  ...
  `GpuPassDesc.NoColor` carries the distinction.
...
- **A demo cannot show a `NoTarget()` depth-only pass working on the desktop**,
  for the HAL reason recorded under [Passes](#passes). `cameras` emits one, and
  what it demonstrates on desktop is the tag filter deciding that exactly one
  material participates; what it demonstrates in the browser is the pass
  executing. Both halves are needed, and the browser half is the only place the
