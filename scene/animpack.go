...
// animates reports whether the model has anything for a draw to play.
func (a *residentAnimation) animates() bool { return a.jointCount > 0 }

// skin reports the group 2 buffers a draw of this model binds. A model with no
// joints binds none of its own and leaves the pair unbound, which is what puts
// the draw on the shared null skin the flush fills in.
func (a *residentAnimation) skin() skinBuffers {
	if !a.animates() {
		return skinBuffers{}
	}
	return skinBuffers{poses: a.poses, joints: a.skinJoints, bound: true}
}
...
func resolvePlays(
	...
	dst []scenePlayRecord, report reportOnce,
) []scenePlayRecord {
	if !anim.animates() || len(plays) == 0 {
		return dst
	}
	...
	for i := range count {
		dst = append(dst, kept[i].record(anim.jointCount, anim.sampleRate, kept[i].weight/total))
	}
	return dst
}
...
// record turns one resolved play into the two rows and two folded weights the
// shader reads.
//
...
func (p resolvedPlay) record(jointCount, sampleRate int, weight float32) scenePlayRecord {
	time := p.time
	switch {
...
func (b *frameBuild) packAnim(plays []scenePlayRecord) uint32 {
	if len(plays) == 0 {
		return sceneNoAnim
	}
	offset := len(b.anims.bytes()) / 16
	header := sceneAnimHeader{PlayCount: uint32(len(plays))}
