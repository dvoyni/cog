...
// first-wins.
func (p *plugin) shadeSprite(materials *internal.ScopeMaterials, material *gfx.MaterialDescr, fingerprint uint64, params []gfx.ParameterDescr) spriteShading {
	shading := spriteShading{}
	var scope []gfx.ParameterDescr
	shading.material, shading.fingerprint, scope = materials.Resolve(internal.FamilySprite, material, fingerprint)
...
func (p *plugin) shadeTriangles(materials *internal.ScopeMaterials, op *internal.TrianglesOp) trianglesShading {
	f := internal.FamilyTriangles
	if op.Unkeyed {
		f = internal.FamilyTexture
	}
