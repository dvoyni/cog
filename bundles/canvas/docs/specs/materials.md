...
`github.com/dvoyni/cog/bundles/canvas` draws every 2D thing the engine puts on screen —
...
- **The `SpriteInstance` record** — struct name, member names, order and size:
  ...
  It is hand-mirrored by `canvas.SpriteInstance` (`bundles/canvas/shader.go`) and
...
(`extensions/gfx/limits.go:9-10`, `extensions/wgpu/gfxbackend.go:475`), so one `u32` component buys the
...
impossible: each member owns its own `@location`, and `bundles/canvas/batch.go` writes
...
(`extensions/gfx/limits.go:11-43`) counts storage buffers, bind groups and uniform size
...
parameter that is a Go struct is the `bundles/scene/pack.go` hazard again — smaller and
...
**First wins.** `parameterRefFor` (`extensions/gfx/parameterplan.go:119-130`) scans a draw's
...
- A shape helper's `Color` beats a caller's `tint` in the draw parameters,
  ...
  `Color`.
- **`DrawTexture`'s doc comment (`bundles/canvas/opqueue.go:263`) is wrong and is
...
`MaterialDescr.Fingerprint()` (`extensions/gfx/material.go:87`) is the key, and canvas
...
`CloneTo` into `materialArena` (`bundles/canvas/opqueue.go:154`), and store the `uint64`
...
reflected at `extensions/wgpu/gfxreflect.go:46` and read by nothing outside tests — so the
...
`scenePbrRecord` (`bundles/scene/pack.go:109`) hand-mirrored against WGSL and sized with
...
- **`Element.Material(canvas.MaterialSet) Element`**, a **Modifier** in
  ...
  (`bundles/ui/layout.go:150-155`) and stamped into `State` beside `Layer` (`:819`).
