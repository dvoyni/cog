...
canvas queue during its own processing (`bundles/ui/plugin.go:80`). That is why
...
handler (`bundles/canvas/plugin.go:105`), during `app.UpdateEvent`, long before
...
valid "until the queue is reset or recorded into again" (`bundles/canvas/inspect.go:72`).
...
Each op carries `canvas.Op`'s fields (`bundles/canvas/inspect.go:24-52`) **minus the gfx
...
> `RectView` in `bundles/canvas/snapshot.go`, built through the same rules — one value
...
[bundles/ui/docs/specs/mcp.md §Three coordinate spaces](../../../ui/docs/specs/mcp.md#three-coordinate-spaces).
...
**`bundles/canvas/snapshot.go`** (new)
