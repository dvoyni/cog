...
func TestOpsReturnRecordedOperationsInFlushOrder(t *testing.T) {
	...
	write.SetLayerTransform(2, m.Rect{Width: 200, Height: 100}, canvas.AspectOverlap)
	write.Text(5, "font.ttf", "score", canvas.TextDraw{Position: m.Vec2{X: 4}, Size: 12})
	write.SetClip(m.Rect{X: 1, Y: 2, Width: 3, Height: 4})
	write.Sprite(2, "hero.png", canvas.SpriteTransform{Scale: 2, FlipX: true, TileY: true},
		nil, gfx.ColorParam("tint", m.Color{R: 1, A: 1}))
	write.RemoveClip()
	write.DrawTriangles(2, []canvas.Vertex{
	...
	if ops[0].Kind != canvas.OpSprite || ops[0].Layer != 2 || ops[0].Path != "hero.png" {
	...
	if ops[1].Kind != canvas.OpTriangles || len(ops[1].Vertices) != 3 || ops[1].Vertices[2].Position.X != 3 {
		t.Fatalf("second op = %+v, want the recorded triangle list", ops[1])
	}
	if ops[1].HasClip {
		t.Fatal("triangles kept a removed clip")
	}
	if ops[2].Kind != canvas.OpText || ops[2].Layer != 5 || ops[2].Text != "score" || ops[2].FontPath != "font.ttf" {
		t.Fatalf("third op = %+v, want the layer 5 text", ops[2])
	}

	window, aspect, ok := write.LayerWindow(2)
	if !ok || window != (m.Rect{Width: 200, Height: 100}) || aspect != canvas.AspectOverlap {
...
func TestOpsOmitVerticesOfCustomVertexLayouts(t *testing.T) {
	...
	ops := write.Ops(nil)
	if len(ops) != 1 || ops[0].Kind != canvas.OpTriangles {
...
func TestOpsReportTheTextureASpriteAndACustomShapeSource(t *testing.T) {
	...
	write.DrawTexture(1, texture, []canvas.Vertex{
		{Position: m.Vec2{}, Color: white},
		{Position: m.Vec2{X: 4}, Color: white},
		{Position: m.Vec2{Y: 4}, Color: white},
	}, nil)
	write.FillRect(2, m.Rect{Width: 5, Height: 5}, canvas.ShapeDraw{Color: white})

	ops := write.Ops(nil)
	if len(ops) != 3 {
		t.Fatalf("ops = %d, want 3", len(ops))
	}
	if width, height := ops[0].Texture.Size(); ops[0].Kind != canvas.OpSprite || ops[0].Path != "" || width != 8 || height != 4 {
		t.Fatalf("sprite op = %+v, want the recorded texture and no path", ops[0])
	}
	if ops[1].Kind != canvas.OpTriangles || ops[1].Texture.ID() != texture.ID() {
...
func TestLayerTargetReportsWhereALayerDraws(t *testing.T) {
	...
	write.FillRect(3, m.Rect{Width: 5, Height: 5}, canvas.ShapeDraw{Color: m.Color{A: 1}})
	write.FillRect(4, m.Rect{Width: 5, Height: 5}, canvas.ShapeDraw{Color: m.Color{A: 1}})
