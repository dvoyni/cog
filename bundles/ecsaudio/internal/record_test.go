...
//
// The cap is one Voice, so the second Emitter's Play is the play that loses. It
// still got a real handle and its ending arrives in the flush that recorded it,
// which is what makes this reachable without waiting for anything to be quiet.
func TestAStolenVoiceNeverRestarts(t *testing.T) {
	...
	second := h.emit(sound.ClipWithResource(clip), sound.Params{})
	h.tick()

	if ended := h.waitEnded(); ended.Reason != sound.ReasonStolen {
		t.Fatalf("the play that met a full table ended as %v, want stolen", ended.Reason)
	}
	got := h.probe(second)
	if got.Found {
		t.Fatalf("the stolen play is in the live view as %v", got.Entry.voice)
	}
	if !got.Held {
	...
	after := h.probe(second)
	if after.Entry.voice != stolen {
		t.Fatalf("a stolen Voice restarted: the entry moved from %v to %v", stolen, after.Entry.voice)
	}
	if after.Live != 1 {
		t.Fatalf("a stolen Voice restarted: %d live Voices, want only the one that kept its slot", after.Live)
	}
	h.noEnding()
	h.noErrors()
}
