...
//
// It is mounted at builtin/model/frame.wgsl and published as model.FramePath,
// so a custom material includes it by that absolute storage name and lights
// with the frame scene packs rather than a hand-copied prefix of it.
// sceneFrame is the one binding it costs, and scene binds it on every draw. The SCENE_MAX_LIGHTS default is model.MaxLights, so an includer
// supplies nothing.
