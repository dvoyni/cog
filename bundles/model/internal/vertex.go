...
// into the storage layout at bake (model/internal/vertexpack.go), so what a
// shader reads is that layout's offsets and formats rather than this struct's:
// 72 bytes of float are authored here and 32 are stored, because four of the
// six rows narrow - the normal, the tangent and both UV sets to four bytes
// each - and the colour quantises to a unorm byte a channel. An app writes
// directions in the m.Vec3 and m.Vec4 it would write anyway and never sees the
// encoding.
...
// weights that sum to exactly one - see bundles/model/internal/vertexskin.go.
//
