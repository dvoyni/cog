...
`github.com/dvoyni/cog/bundles/scene` holds every mesh it draws in three buffers: an
...
not an accident: `bundles/scene/mesh.go:13-21` explains the size with a reason that was
...
(`bundles/scene/mesh.go:238`) and `appendStaging` copies every byte into the staging
arena (`bundles/scene/meshbake.go:168`). Packing replaces that memcpy with a
...
(`bundles/scene/animpack.go:57`), and `bundles/scene/mesh.go:18` already recorded that
...
is already `m.Color` (`bundles/canvas/types.go:16`), so the vocabulary precedent exists;
its limit is worth recording accurately, since canvas *stores* `Float32x4`
(`bundles/canvas/shader.go:148`) and is a precedent for the type, not for packing it.
...
`generateTangents` (`bundles/scene/gltfmesh.go:336-378`) fires only when the material has
...
face rig), and `bundles/scene/gltfmesh.go:157` already promises per-model joint
...
> (`bundles/scene/gltfanim.go:178-195`), so **plain** joints are appended after the skin
...
- **The ranges cost no new traversal.** `readAttribute` already visits every UV
  element on the glTF path, and `vertexBounds` (`bundles/scene/mesh.go:202-206`) already
...
`BenchmarkPackVertices` in `bundles/scene/vertexpack_test.go`, at `-benchtime 300x
-count=7`, medians:
...
(`extensions/gfx/translate.go:33`).
...
- **`variantFor` moves from the model to the primitive.**
  `owned.variants[variantFor(skin.bound, skin.morphed)]` (`bundles/scene/model.go:297`)
...
`bundles/scene/material.go:353-361`).
...
`sceneNoSkin` flag (`bundles/scene/pack.go:252`) and `neverCull` (`bundles/scene/model.go:342`).
...
below the vertex count — `validateMesh` (`bundles/scene/mesh.go:213`) enforces it on the
authoring path and the glTF path has it by construction. That matters:
`bakeModelGeometry` (`bundles/scene/modeltable.go:335`) never calls `mintMesh`, so a
...
`indexBytes` (`bundles/scene/mesh.go:307`) is a **zero-copy reinterpret** of `[]uint32`
...
(`bundles/scene/gltfmesh.go:198`), and the only strip in either repo is non-indexed.
...
`UpdateMesh` freezes the *layout* (`bundles/scene/meshbake.go:113`) because layout is part
...
- **The loader never reorders vertices to tighten spans.** This is the first
  ...
  with both the unweld permutation (`bundles/scene/gltfmorph.go:143`) and
...
comparison (`bundles/scene/mesh.go:147`) and which already gates two things: whether a
...
> (`bundles/scene/mesh.go:40-48`). Under the divergence those offsets are the *authoring*
...
- **`bundles/scene/unitmesh.go:91,158`** — `Color` changes type. Two lines;
...
against the tree: every `@location` in `bundles/scene/builtin` and `bundles/canvas/builtin` is
...
`gfx` derives the stride as the largest attribute end offset (`extensions/gfx/mesh.go:138`)
...
(`extensions/gfx/mesh.go:132`) and validates nothing; with two widths, a buffer declared
...
**No ADR.** [Assemble bundles/scene/docs/specs/mesh.md](https://github.com/dvoyni/cog/issues/175)
...
- Index-length validation in the translator, at the existing
  ...
  (`extensions/gfx/translate.go:366`) — `MeshIndexed` is a pure value constructor with no
...
**`bundles/scene/gltfload.go`, `bundles/scene/gltfanim.go`, `bundles/scene/gltfmesh.go`**
...
**`bundles/scene/model.go`, `bundles/scene/pack.go`, `bundles/scene/modeltable.go`**
...
- `bundles/scene/docs/specs/scene.md:1261-1300` collapses to a pointer here. That
...
- **Deriving the tangent frame per-fragment from UV derivatives, storing none at
  ...
  mesh stores* one, against `bundles/scene/gltfmesh.go:343-344`'s own note that a
