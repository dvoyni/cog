...
func NewLookupReadAccess(lookup *Lookup) LookupReadAccess {
	return internal.NewLookupReadAccess(lookup)
}
...
func NewMeshRef(source MeshSource, id, generation uint32) MeshRef {
	return internal.NewMeshRef(source, id, generation)
}
...
func MintMesh[TVertex VertexLayout](
	...
	return internal.MintMesh[TVertex](cache, arena, vertices, indices, topology, durable)
}
...
func NewClipMachine(clips []ClipInfo, states []ClipState, transitions []ClipTransition) (ClipMachine, error) {
	return internal.NewClipMachine(clips, states, transitions)
}

// VariantFor picks the shader variant a draw needs from what it deforms.
func VariantFor(skinned, morphed bool) ShaderVariant { return internal.VariantFor(skinned, morphed) }
...
func ResolvePlays(
	...
	return internal.ResolvePlays(anim, path, plays, dst, weights, report)
}
...
func BlendJoint(anim *ResidentAnimation, plays []ScenePlayRecord, joint int) (m.Mat4, bool) {
	return internal.BlendJoint(anim, plays, joint)
}
...
func BlendMorphWeights(
	...
	return internal.BlendMorphWeights(anim, path, plays, frames, override, overridden, dst, report)
}
...
func SelectMorphTargets(
	...
	return internal.SelectMorphTargets(weights, dst, path, report)
}
...
func BundledIngredients(defaults PbrDefaults) MaterialIngredients {
	return internal.BundledIngredients(defaults)
}
...
func VariantShader(shader gfx.ShaderDescr, variant ShaderVariant) gfx.ShaderDescr {
	return internal.VariantShader(shader, variant)
}

// SceneShader describes the bundled PBR module under the given options.
func SceneShader(opts ...gfx.ShaderOption) gfx.ShaderDescr { return internal.SceneShader(opts...) }
...
func PackVertices(arena *[]byte, vertices []Vertex) (Span, m.Sphere, SceneMesh) {
	return internal.PackVertices(arena, vertices)
}
...
func PackInstance(world m.Mat4, anim InstanceAnim, mesh uint32) Instance {
	return internal.PackInstance(world, anim, mesh)
}
...
func PackLight(descr LightDescr) (Light, error) { return internal.PackLight(descr) }
...
func ContributionAt(light *Light, point m.Vec3) float32 {
	return internal.ContributionAt(light, point)
}
...
func PackFrameLighting(block FrameBlock, lighting FrameLighting, selection *LightSelection) FrameBlock {
	return internal.PackFrameLighting(block, lighting, selection)
}
...
func AppendAnim(dst []byte, plays []ScenePlayRecord, morph AnimMorph) ([]byte, uint32) {
	return internal.AppendAnim(dst, plays, morph)
}
