...
- **Measured against the stop before it**, interleaved with an A/A copy of the
  ...
  - **Gap:** `BenchmarkTheStep` at N = 1 024 reads 2.6 to 3.0% slower in four
    rounds of 10 to 12, against an A/A spread of 0 to 0.9%. Its scene engages
    no Body in 3 000 ticks, so the rule never runs there. The same slowdown
    shows with the rule compiled in but unreachable, and does not show with the
    Slop's plumbing alone (+0.7%). That points at code placement, not work, but
    it is not confirmed.
