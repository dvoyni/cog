...
func TestAIsTheNonStaticPartyAndTheNormalFacesIt(t *testing.T) {
	...
	if got := entry.NormalFor(wall); !solverNear(got.Y, -1) {
...
func TestAPairThatFlickersApartAndBackKeepsItsImpulses(t *testing.T) {
	...
	if !solverNear(revived.Points[0].NormalImpulse, 17.5) || !solverNear(revived.Points[0].TangentImpulse, -2.25) {
...
func solverNear(got, want float64) bool { return math.Abs(got-want) <= oracleTolerance }
