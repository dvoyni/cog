...
// the seven parameter slots mean anything. Every one of the ten constructors
// fills its own slots, and the per-kind accessors read and write them; the
// slots are exported only so a Joint serialises whole, and are not written
// directly.
//
...
//
// The parameters carry an invariant with Kind, as Shape's vertices do. They
// are exported, as every Component field is, so a Joint serialises whole, and
// are not written directly: a constructor and the per-kind accessors are how
// they change. Everything else is a plain field, so the three solver constants
// and the collision flag are data the app writes. A Joint written as
...
type Joint struct {
	...
	// Params are the Kind's parameter slots. Do not write them directly: a
	// constructor fills them and the per-kind accessors change them.
	Params [7]float64
	...
	// AccumulatedImpulse is the solver's warm-starting state, which it writes
	// every tick; Impulse reads it as the Kind's magnitude. Do not write it.
	AccumulatedImpulse m.Vec2d
...
func (j Joint) Impulse() float64 {
	...
	case JointPivot, JointGroove:
		return j.AccumulatedImpulse.Length()
	case JointSpring, JointRotarySpring:
		return j.AccumulatedImpulse.X
	default:
		return math.Abs(j.AccumulatedImpulse.X)
	}
}
