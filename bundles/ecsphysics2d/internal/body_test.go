...
func TestIntegratingAVelocitySpendsTheForceAndClearsIt(t *testing.T) {
	...
	if want := (m.Vec2d{X: 10 * solverStep / 2, Y: -4 * solverStep / 2}); !vecNear(velocity.Linear, want) {
		t.Errorf("Linear = %v, want %v", velocity.Linear, want)
	}
	if want := 16 * solverStep / 8; !nearF(velocity.Angular, want) {
...
func TestGravityIsAddedBesideTheForceAndScaledByTheStep(t *testing.T) {
	...
	IntegrateVelocity(&body, &velocity, &force, gravity, solverStep)

	linear, angular := math.Exp(-15*solverStep), math.Exp(-30*solverStep)
	want := m.Vec2d{X: 4*linear + (0.4+10.0/2)*solverStep, Y: 1*linear + (-9.81-4.0/2)*solverStep}
...
func TestIntegratingAPositionRecordsWhereTheBodyWas(t *testing.T) {
	...
	IntegratePosition(&position, &velocity, solverStep)
	...
	if want := (m.Vec2d{X: 1 + 6*solverStep, Y: 2 - 12*solverStep}); !vecNear(position.Current, want) {
		t.Errorf("Current = %v, want %v", position.Current, want)
	}
	if want := 0.5 + 3*solverStep; !nearF(position.Angle, want) {
...
func TestDampingIsExponentialInTheRatePerSecond(t *testing.T) {
	...
	if want := 4 * math.Exp(-15*solverStep); !nearF(velocity.Linear.X, want) {
		t.Errorf("Linear.X = %v, want %v", velocity.Linear.X, want)
	}
	if want := 2 * math.Exp(-30*solverStep); !nearF(velocity.Angular, want) {
		t.Errorf("Angular = %v, want %v", velocity.Angular, want)
	}
}
...
func TestABodyUnderAConstantForceRampsToItsTerminalSpeed(t *testing.T) {
	...
	terminal := asked * rate * solverStep / (1 - math.Exp(-rate*solverStep))
	...
	for tick := 1; tick <= 600; tick++ {
		...
		if velocity.Linear.X >= 0.95*asked && math.IsInf(reached, 1) {
			reached = float64(tick) * solverStep
		}
	}
...
func TestACrateStopsByDampingWithAHalfLifeOfLnTwoOverTheRate(t *testing.T) {
	...
	for tick := 1; tick <= 600; tick++ {
		...
		if velocity.Linear.X <= 2 && math.IsInf(half, 1) {
			half = float64(tick) * solverStep
		}
	}

	want := math.Ln2 / rate
	t.Logf("half-life %.4f s against ln2/%v = %.4f s", half, rate, want)
	if math.Abs(half-want) > solverStep {
		t.Errorf("the half-life is %v s, want %v s within one step of %v s", half, want, solverStep)
	}
}
...
func TestTheIntegratorsAllocateNothing(t *testing.T) {
	...
	if got := testing.AllocsPerRun(1000, func() {
		...
		IntegrateVelocity(&body, &velocity, &force, m.Vec2d{X: 0.4, Y: -9.81}, solverStep)
		IntegratePosition(&position, &velocity, solverStep)
	}); got != 0 {
