...
func TestTheCachedSimplexWarmStartsTheNextTickAndAnswersTheSame(t *testing.T) {
	...
	for name, cached := range map[string]uint32{
		...
		for i := range cold.count {
			if !solverNear(warm.points[i].depth, cold.points[i].depth) {
...
func TestAPolygonVertexOnTheLineOfAnotherPolygonsEdgeAndBeyondItDoesNotTouch(t *testing.T) {
	...
	solverStep := NewBoxShape(1, 1, 0.05)
	...
	for _, gap := range supportingLineGaps {
		...
		for i := range supportingLineAngles {
			...
			_, depth, ok = Penetration(solverStep, m.Vec2d{}, angle, nil, solverStep, atB, angle, nil)
			_, backDepth, backOK = Penetration(solverStep, atB, angle, nil, solverStep, m.Vec2d{}, angle, nil)
			report("the two steps", gap, angle, depth, backDepth, ok, backOK)

			_, boxA := finitenessWorld(finitenessShape{shape: solverStep}, m.Vec2d{}, angle)
			_, boxB := finitenessWorld(finitenessShape{shape: solverStep}, atB, angle)
			if boxA.Intersects(boxB) {
				gated++
				detected += detectTwoBodies(solverStep, m.Vec2d{}, angle, nil, solverStep, atB, angle, nil)
			}
		}
	}
