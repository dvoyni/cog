...
// Restitution.
func withMaterial(friction, restitution float64) Shape {
...
func TestOneSlipperyShapeSlidesThePairAndOneDeadShapeAbsorbsTheBounce(t *testing.T) {
	...
	// bounce.
	slippery := detectPair(t, withMaterial(0, 1), withMaterial(1, 1))
	if slippery.Friction != 0 {
		t.Errorf("a frictionless Shape against a gripping one has Friction %v, want 0",
			slippery.Friction)
	}
	dead := detectPair(t, withMaterial(1, 0), withMaterial(1, 1))
...
func TestNeitherMaterialIsValidatedAnywhere(t *testing.T) {
	...
	entry := detectPair(t, withMaterial(1.5, 1.25), withMaterial(2, 0.8))
	if !near(entry.Friction, 3) {
		t.Errorf("a pair of 1.5 and 2 has Friction %v, want 3", entry.Friction)
	}
	if !near(entry.Restitution, 1) {
