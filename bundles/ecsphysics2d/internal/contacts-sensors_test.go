...
func TestAMovingCircleSensorIsProbedAlongItsPathAndNotOnlyWhereItLanded(t *testing.T) {
	...
	bodies.InsertMoving(sensor, solverSensorCircle(0), m.Vec2d{X: 4}, m.Vec2d{}, 0, 0, nil)
	...
	discrete.Insert(sensor, solverSensorCircle(0), m.Vec2d{X: 4}, 0, nil)
...
func TestAProbedSensorThatStartedInsideSomethingReportsTheOverlapAtTheStart(t *testing.T) {
	...
	bodies := NewBodyIndex(0)
	bodies.InsertMoving(sensor, solverSensorCircle(0.5), m.Vec2d{X: 2}, m.Vec2d{}, 0, 0, nil)
...
func TestASensorThatDidNotMoveIsTestedWhereItStands(t *testing.T) {
	...
	bodies := NewBodyIndex(0)
	bodies.InsertMoving(ecs.Entity(1), solverSensorCircle(0.5), m.Vec2d{X: 5}, m.Vec2d{X: 5}, 0, 0, nil)
...
func TestTwoMovingSensorsThatFindEachOtherKeepTheSmallerT(t *testing.T) {
	...
	for _, each := range cases {
		t.Run(each.name, func(t *testing.T) {
			bodies := NewBodyIndex(0)
			bodies.InsertMoving(first, solverSensorCircle(0.1), each.toFirst, each.fromFirst, 0, 0, nil)
			bodies.InsertMoving(second, solverSensorCircle(0.1), each.toSecond, each.fromSecond, 0, 0, nil)
...
func TestASweptSensorPairBeginsThenContinuesThenEndsExactlyOnce(t *testing.T) {
	...
	sweep := func(from, to m.Vec2d) {
		bodies.Clear()
		bodies.InsertMoving(sensor, solverSensorCircle(0), to, from, 0, 0, nil)
	}
...
func TestTheSweptSensorPathAllocatesNothing(t *testing.T) {
	...
	fill := func() {
		...
		for i := range 16 {
			...
			bodies.InsertMoving(ecs.Entity(1+i), solverSensorCircle(0.05), to, from, 0, 0, nil)
		}
	}
...
func solverSensorCircle(radius float64) Shape {
