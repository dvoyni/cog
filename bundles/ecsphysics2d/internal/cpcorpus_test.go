...
func cpAgrees(want cpCollideCase, got touching, ok bool) bool {
	...
	solverNear := func(got, want float64) bool {
		return !math.IsNaN(got) && !math.IsNaN(want) && math.Abs(got-want) <= cpTolerance
	}
	if !solverNear(got.normal.X, want.normal[0]) || !solverNear(got.normal.Y, want.normal[1]) {
		return false
	}
	for i := range got.count {
		if !solverNear(got.points[i].p1.X, want.points[i][0]) ||
			!solverNear(got.points[i].p1.Y, want.points[i][1]) ||
			!solverNear(got.points[i].p2.X, want.points[i][2]-shift*want.normal[0]) ||
			!solverNear(got.points[i].p2.Y, want.points[i][3]-shift*want.normal[1]) ||
			!solverNear(got.points[i].depth, want.points[i][4]+shift) {
			return false
		}
	}
	return true
