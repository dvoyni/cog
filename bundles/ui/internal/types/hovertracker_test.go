...
func hoverFrame(id ID) *Interactions {
	...
	processor.Process(canvas.LookupAccess{}, FrameRoots(&frame), FrameLayers(&frame),
		GlobalState{Screen: Rect{Width: 10, Height: 10}, Pointer: PointerState{X: 5, Y: 5}}, nil)
	PublishInteractions(&processor, interactions)
	return interactions
}
