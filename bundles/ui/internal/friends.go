...
// The friend functions: what uiimpl, and the root's own tests, read from or do
// to a public type's unexported state. Only the ui root and uiimpl can import
...
// FrameRoots reads Frame.roots for uiimpl: the roots declared this tick, in
// declaration order.
func FrameRoots(frame *Frame) []Element { return frame.roots }

// FrameLayers reads Frame.layers for uiimpl: the base layer of the root at the
// same index of FrameRoots.
func FrameLayers(frame *Frame) []canvas.Layer { return frame.layers }

// FrameMaterials reads Frame.materials for uiimpl: the material set every root
// of this tick inherits.
func FrameMaterials(frame *Frame) canvas.MaterialSet { return frame.materials }

// ClearFrame calls Frame.clear for uiimpl, which consumes the frame at the end
