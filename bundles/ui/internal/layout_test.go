...
func TestProcessGivesOverlappingInteractionsToTopmostOnly(t *testing.T) {
	...
	typesAssertInteractions(t, context.interactions, []Interaction{
...
func TestCoveringElementBlocksElementsBelowIt(t *testing.T) {
	...
	var context Processor
	context.Process(canvas.LookupAccess{}, roots, layers, state, nil)
	if typesHasInteraction(context.interactions, "button", InteractionDown) ||
		typesHasInteraction(context.interactions, "button", InteractionClick) ||
		typesHasInteraction(context.interactions, "button", InteractionHover) {
		t.Fatalf("blocked button interactions = %+v, want none", context.interactions)
	}
	if !typesHasInteraction(context.interactions, "shade", InteractionClick) {
...
func TestIgnoreClipLetsPopupEscapeParent(t *testing.T) {
	...
	if typesHasInteraction(context.interactions, "child", InteractionHover) {
		t.Fatal("clipped child unexpectedly received hover")
	}
	context.Process(canvas.LookupAccess{}, popup, nil, state, nil)
	if !typesHasInteraction(context.interactions, "popup", InteractionHover) {
...
func typesAssertInteractions(t *testing.T, got, want []Interaction) {
	t.Helper()
...
func typesHasInteraction(interactions []Interaction, id ID, kind InteractionKind) bool {
