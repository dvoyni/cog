...
**`Element.userData` is `any` (`bundles/ui/element.go:116`) and cannot be cloned.** A ui
...
- `Frame.Add` says the root is copied but "descendant slices may remain
  borrowed" (`bundles/ui/frame.go:22`), and `Element.children` is that borrow
  (`bundles/ui/element.go:134`).
- `frame.clear()` runs as a `defer` at the end of `processUpdate`
  (`bundles/ui/plugin.go:55,63`), so the frame truncates every tick.
...
`processor` is a resource (`bundles/ui/plugin.go:29`) holding `nodes []layoutNode`
(`bundles/ui/layout.go:85-95`) with `rect`, `clip`, `childrenClip`, `layer`, `order` and
...
[bundles/canvas/docs/specs/mcp.md §Where it sits in the tick](../../../canvas/docs/specs/mcp.md#where-it-sits-in-the-tick).
...
tracks `subtreeEnd` (`bundles/ui/layout.go:140-150`, `:88`), so containment is readable
from indices alone.
...
> (`bundles/ui/layout.go:180`), and it is never rewritten: reporting it verbatim would
...
`Element` (`bundles/ui/element.go:118-135`) — but **only when set**. Those fields already
...
> `opt[size]`** (`bundles/ui/element.go:123`). The declared block therefore carries
...
`Element.id` is **optional** (`bundles/ui/element.go:118`, set by `.ID()`), and
`Interactions.Has` matches by **prefix** (`bundles/ui/interactions.go:39-40`). Ids are
...
and breaks on a HiDPI laptop. `pointerToViewport` (`bundles/ui/plugin.go:94-104`) scales
...
`processUpdate` ends in `defer frame.clear()` (`bundles/ui/plugin.go:55`), so between
...
>
> - **`bundles/ui/commandsimpl.go`** (new) holds `armLayoutCmdImpl` and ui's
...
> - **`bundles/ui/err.go`** (new) holds `ErrLayoutBusy`, `ErrLayoutAbandoned` and
...
the layout container (`bundles/ui/containers.go:4`).
...
- **The hit test swallows rather than falling through.** "Every element absorbs
  ...
  through" (`bundles/ui/layout.go:955-963`). An id-less overlay element is not harmless:
  it eats clicks over its rect, including the agent's own synthetic ones.
  `IgnoreHitTest()` (`bundles/ui/modifiers.go:281`) fixes it, on every element, every
...
- **Expressiveness had no cheap answer.** A faithful projection of ui's grammar
  ...
  (`ColorPanel` is the only fill, `bundles/ui/elements.go:440`), so a hollow box is four
...
- **[ui: an empty `Font.Path` draws nothing instead of the built-in
  ...
  stays open on its own merits — `bundles/ui/elements.go:478-492` contradicts the doc
  three lines above it, and `canvas` built the fallback deliberately for debug
  text (`bundles/canvas/font.go:44-54`). It is no longer a precondition of anything
