...
`mcp.Provider` and contributes its own, one `Start`, one `Stop`, and one
capability of its own. Its name is `mcp.Name`, `"mcpserver"`.
...
nothing; an Adapter nobody collects is not an error.
...
1. Reads the bound providers, each with the `PluginName` that contributed it.
...
started. The broker declares no dependencies, collecting its Adapters adds none,
and `orderPlugins` is stable in the app author's listing order
(`kernel/engine.go:160-180`), so the broker typically starts *early* — before,
say, `canvas.Start` mounts the built-in font (`bundles/canvas/plugin.go:71`). A tool call
...
**Tool name** is `<plugin>_<capability>`, taken from the `PluginName`
`CollectAdapters` records for the plugin that contributed the Provider, and
`Capability.Name()`. A Provider carries no name, so the prefix cannot be forged.
Underscore, because the MCP name charset is conservative and `_` is the one
separator no client rejects. Uniqueness across plugins is inherited from the
engine's own duplicate-plugin-name rejection (`ErrConflictingPluginName`,
`kernel/engine.go:77`). A plugin may contribute more than one Provider, so a
duplicate name is checked per plugin, across all of them.
...
The broker contributes an `mcp.Provider` of its own, offering exactly one
capability.
...
- Two providers offering the same capability name compose fine (the plugin
  prefix separates them); one plugin offering it twice fails, whether from one
  Provider or from two.
- An engine composed without the broker runs with every provider contributing.
