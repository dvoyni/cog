...
// are tested in internal, where the authority is declared; what is tested here
// is what it owes a Store, which exists only in this package.
...
func TestDespawnEmptiesEveryStore(t *testing.T) {
	...
	if !internal.EntitiesDespawn(entities, doomed) {
...
func TestRecyclingAnIndexLeaksNoDenseEntry(t *testing.T) {
	const cycles = 1000
	entities := internal.NewEntities(1)
	...
	for i := range cycles {
		...
		if !internal.EntitiesDespawn(entities, e) {
...
func TestNoIdentifierInTheBundleIsAWorld(t *testing.T) {
	var sources []string
	for _, pattern := range []string{"*.go", "internal/*.go", "ecsimpl/*.go"} {
