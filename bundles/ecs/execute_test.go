...
func TestAResponseOfTheWrongTypeIsRejected(t *testing.T) {
	defer func() {
		...
		message, _ := recovered.(string)
		for _, want := range []string{"ecs.Resp[github.com/dvoyni/cog/bundles/ecs.body]", "ecs.nudgeResponse"} {
			if !strings.Contains(message, want) {
				t.Fatalf("panic %v does not name %q", recovered, want)
			}
		}
...
func TestASystemNamingTheResponseTwiceIsRejected(t *testing.T) {
	defer func() {
		...
		message, _ := recovered.(string)
		for _, want := range []string{"ecs.Resp[github.com/dvoyni/cog/bundles/ecs.nudgeResponse]", "more than once"} {
			if !strings.Contains(message, want) {
				t.Fatalf("panic %v does not name %q", recovered, want)
			}
		}
