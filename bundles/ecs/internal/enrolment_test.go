...
func TestDespawnEmptiesEveryStore(t *testing.T) {
	...
	positions := NewStore[typesPosition](entities, 8)
	tags := NewStore[disabled](entities, 8)
	doomed, bystander := entities.alloc(), entities.alloc()
	positions.Set(doomed, typesPosition{X: 1})
	tags.Set(doomed, disabled{})
	positions.Set(bystander, typesPosition{X: 2})
...
func TestRecyclingAnIndexLeaksNoDenseEntry(t *testing.T) {
	...
	store := NewStore[typesPosition](entities, 1)

	peak := 0
	for i := range cycles {
		e := entities.alloc()
		store.Set(e, typesPosition{X: float32(i)})
		if store.Len() > peak {
			peak = store.Len()
		}
