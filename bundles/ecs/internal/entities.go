...
type Entities struct {
	...
	//
	// It is the Store's remove bound to the Store rather than the Store behind
	// an interface, because the Store is declared in the ecs root, which this
	// package cannot name, and an interface method a root type can satisfy from
	// here would have to be exported on every *Store[T]. Either way a despawn
	// pays one indirect call per Store, never per entity.
	...
	//
	// The value is the ecs root's own record, held as any because this package
	// cannot name the root's type. Only the root stores one and only the root
	// reads one back, so the assertion is registration-time work and nothing a
	// tick pays.
	classes map[reflect.Type]any
}
...
func (en *Entities) declare(componentType reflect.Type, class any) {
	if en.classes == nil {
		en.classes = map[reflect.Type]any{}
	}
...
func (en *Entities) classOf(componentType reflect.Type) any {
