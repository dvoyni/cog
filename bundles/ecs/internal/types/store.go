...
type Store[T any] struct {
	...
	trivial bool
	// hooks is PROTOTYPE (proto/ecs-hooks): this Store's Hook log, nil unless
	// some Hooks[T, K] reads it, which is the nil check an unobserved Store pays.
	hooks *hookLog[T]
...
func (s *Store[T]) Set(e Entity, value T) {
	...
	s.add(e, value)
	if s.hooks != nil {
		s.hooks.added(e, kindAdded|kindChanged)
	}
}
