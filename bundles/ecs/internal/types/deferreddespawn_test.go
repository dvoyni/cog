...
func newDeferredWorld(t *testing.T) *deferredWorld {
	...
	w.entities, w.components, w.engine = newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.HandleCommand[deferredQueueCmd](ToExecute[deferredRequest, deferredResponse](registrar,
			func(despawn *DeferredDespawn, q *Query[moveQuery], bodies *Get[body]) {
...
func (w *deferredWorld) populated(n int) []Entity {
	...
	for i := range made {
		...
		w.components.velocities.Set(e, velocity{})
...
func seen(q *Query[moveQuery]) []Entity {
	var visited []Entity
	for e := range q.All() {
...
func TestAStaleQueuedDespawnNeverReachesTheEntityThatRecycledItsIndex(t *testing.T) {
	...
	w.components.velocities.Set(recycled, velocity{})
...
func TestAnAppDrainSystemMakesTheChangeVisibleWithinThePublication(t *testing.T) {
	...
	for i := range made {
		...
		components.velocities.Set(made[i], velocity{})
	}
	doomed = made[1]
...
// here.
func deferringCycle(spawn *Spawn[spawnSet], we *WriteableEntities, despawn *DeferredDespawn, q *Query[moveQuery]) {
	for e := range q.All() {
		despawn.Despawn(e)
		break
	}
...
func immediateCycle(spawn *Spawn[spawnSet], we *WriteableEntities, despawn *DeferredDespawn, q *Query[moveQuery]) {
	for e := range q.All() {
		we.Despawn(e)
		break
	}
