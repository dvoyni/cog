...
type splitBuffer struct {
	rows moveQuery
	_    [splitCacheLine - unsafe.Sizeof(moveQuery{})]byte
}
...
// loaded anyway.
type splitBody func(e Entity, it *moveQuery)
...
func (p *splitPool) walkQuery(q *Query[moveQuery], iterations int) {
	q.bind()
...
func (p *splitPool) walkRange(worker, lo, hi int) {
	...
	for row := hi - 1; row >= lo; row-- {
		...
		splitWork((*moveQuery)(buffer), iterations)
	}
}
...
func splitCallWork2(_ Entity, it *moveQuery)   { splitWork(it, splitWork2) }
func splitCallWork25(_ Entity, it *moveQuery)  { splitWork(it, splitWork25) }
func splitCallWork100(_ Entity, it *moveQuery) { splitWork(it, splitWork100) }
func splitCallWork400(_ Entity, it *moveQuery) { splitWork(it, splitWork400) }
...
func splitWork(it *moveQuery, iterations int) {
...
func splitSerialWalk(q *Query[moveQuery], iterations int) {
...
func splitSerialWork2(q *Query[moveQuery])   { splitSerialWalk(q, splitWork2) }
func splitSerialWork8(q *Query[moveQuery])   { splitSerialWalk(q, splitWork8) }
func splitSerialWork15(q *Query[moveQuery])  { splitSerialWalk(q, splitWork15) }
func splitSerialWork25(q *Query[moveQuery])  { splitSerialWalk(q, splitWork25) }
func splitSerialWork100(q *Query[moveQuery]) { splitSerialWalk(q, splitWork100) }
func splitSerialWork400(q *Query[moveQuery]) { splitSerialWalk(q, splitWork400) }
...
func subscribeSplitPool(pool *splitPool, iterations int) func(*kernel.Registrar) {
	return func(registrar *kernel.Registrar) {
		registrar.Subscribe[splitSystem](ToHandler[app.UpdateEvent](registrar,
			func(q *Query[moveQuery]) { pool.walkQuery(q, iterations) }))
	}
}
