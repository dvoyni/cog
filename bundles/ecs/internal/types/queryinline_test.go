...
func querySites(t *testing.T) map[string]*querySite {
	...
	for line := range strings.SplitSeq(inlineVerdicts(t), "\n") {
		...
		at := position[:strings.LastIndex(position, ":")]
...
func TestAllsFillerStaysInsideTheClosureBudget(t *testing.T) {
	...
	for line := range strings.SplitSeq(inlineVerdicts(t), "\n") {
		...
		if shape < 0 {
			t.Errorf("%s: All() carries a walk literal walkShapes does not list: %s", position, name)
			continue
		}
		switch sentence {
		case refused:
			...
			t.Errorf("%s: All()'s shape-%d literal no longer inlines: cost %s exceeds budget %s\n"+
				...
				position, shape, cost, budget, name)
		case accepted:
			...
			if cost > closureBudget {
				t.Errorf("%s: All()'s shape-%d literal costs %d against a budget of %d", position, shape, cost, closureBudget)
			}
		}
	}
	...
	for line := range strings.SplitSeq(inlineVerdicts(t), "\n") {
		...
		case refused:
			t.Errorf("%s: All() no longer inlines: %s\n"+
				...
				"walk inlines anywhere, width 2 included. The Query is %s", position, tail, name)
		case accepted:
			cost, err := strconv.Atoi(strings.TrimSpace(tail))
			if err != nil {
				t.Fatalf("%s: unreadable cost %q", position, tail)
