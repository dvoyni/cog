...
// Twin is types.Twin's twin: another type kernel.TypeName renders as ecs.Twin.
...
type (
	...
	twinQueryCmd  kernel.Command[types.QueryRequest, types.QueryResponse]
	twinCensusCmd kernel.Command[types.CensusRequest, types.CensusResponse]
)
...
func (twins) Dependencies() []kernel.PluginName { return []kernel.PluginName{types.Name} }

func (twins) Register(registrar *kernel.Registrar, _ any) error {
	types.RegisterComponent[types.Twin](registrar, 4)
	types.RegisterComponent[Twin](registrar, 4)
	registrar.HandleCommand[twinSpawnCmd](types.ToExecute[struct{}, struct{}](registrar, func(sp *types.Spawn[twinSet]) {
		sp.New(twinSet{Here: types.Twin{Here: 1}, There: Twin{There: 2}})
	}))
	registrar.HandleCommand[twinQueryCmd](types.QueryCommand)
	registrar.HandleCommand[twinCensusCmd](types.CensusCommand)
	return nil
}
...
func TestAnAmbiguousNameIsRefusedAndItsQualifiedFormsResolve(t *testing.T) {
	here, there := reflect.TypeFor[types.Twin](), reflect.TypeFor[Twin]()
	...
	ambiguous := executioner.ExecuteCommand[twinQueryCmd](types.QueryRequest{Components: []string{"ecs.Twin"}})
	...
	for form, want := range map[string]string{hereForm: `"Here":1`, thereForm: `"There":2`} {
		answer := executioner.ExecuteCommand[twinQueryCmd](types.QueryRequest{Components: []string{form}})
	...
	census := executioner.ExecuteCommand[twinCensusCmd](types.CensusRequest{})
	var twinsListed int
