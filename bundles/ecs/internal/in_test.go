...
// written twice.
func advance(q *Query[typesMoveQuery], dt *In[float64]) {
...
func TestAnInputNamedByNoFeedIsRejected(t *testing.T) {
	message := composeAndFail(t, func(registrar *kernel.Registrar) {
		registrar.Subscribe[guardSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], dt *In[float64]) {}))
	})
...
func TestASystemMayPublishAnEvent(t *testing.T) {
	...
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.Subscribe[noticeSystem](ToHandler[app.UpdateEvent](registrar, func(k kernel.Kernel, q *Query[typesMoveQuery]) {
			count := 0
			for range q.All() {
				count++
			}
