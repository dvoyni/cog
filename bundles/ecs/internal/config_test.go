...
func TestAZeroConfigFieldTakesItsDefault(t *testing.T) {
	for name, value := range map[string]any{"no configuration": nil, "a zero Config": ecs.Config{}} {
	...
	if got, err := resolveConfig(ecs.Config{PrewarmEntities: 50_000}); err != nil || got.PrewarmEntities != 50_000 {
