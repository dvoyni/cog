...
type typesSpawnSystem kernel.Subscription[app.UpdateEvent]
...
func TestASystemSpawns(t *testing.T) {
	entities, components, engine := newWorld(t, 128, func(registrar *kernel.Registrar) {
		registrar.Subscribe[typesSpawnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[spawnSet]) {
			sp.New(spawnSet{Body: body{X: 1, Y: 2}, Velocity: typesVelocity{X: 3}})
		}))
	})
	...
	if value, ok := components.velocities.Get(spawned); !ok || value != (typesVelocity{X: 3}) {
...
func TestASystemDespawns(t *testing.T) {
	...
	components.velocities.Set(doomed, typesVelocity{X: 1})
...
type typesChurnSystem kernel.Subscription[app.UpdateEvent]
...
func TestPopulationHoldsSteadyAcrossHundredsOfTicks(t *testing.T) {
	...
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.Subscribe[typesChurnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[spawnSet], we *WriteableEntities) {
			for _, e := range live {
				if !we.Despawn(e) {
...
func TestSpawnWritesTheAuthorityAndOwnsItsComponents(t *testing.T) {
	_, _, engine := newWorld(t, 16, func(registrar *kernel.Registrar) {
		registrar.Subscribe[typesSpawnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[spawnSet]) { _ = sp }))
	})

	subscription := describeSubscription(t, engine, reflect.TypeFor[typesSpawnSystem]())
	...
	for _, owned := range []string{"body]", "typesVelocity]"} {
	...
	kernel.New(nil).
		...
		WithPlugins(
			...
			&systemsPlugin{subscribe: func(registrar *kernel.Registrar) {
				registrar.Subscribe[typesSpawnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[unregisteredSet]) {}))
			}},
		)
...
func TestAComponentSetFieldIsAComponentValue(t *testing.T) {
	...
	kernel.New(nil).
		...
		WithPlugins(
			...
			&componentsPlugin{ids: 8},
			&systemsPlugin{subscribe: func(registrar *kernel.Registrar) {
				registrar.Subscribe[typesSpawnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[pointerSet]) {}))
			}},
		)
...
func TestATagIsAnOrdinaryComponentSetField(t *testing.T) {
	...
	_, components, engine := newWorld(t, 16, func(registrar *kernel.Registrar) {
		registrar.Subscribe[typesSpawnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[taggedSet]) { sp.New(taggedSet{Body: body{X: 7}}) }))
	})
...
func TestASystemMayRestructureTheEntityItIsVisiting(t *testing.T) {
	...
	t.Run("despawning the current Entity", func(t *testing.T) {
		...
		entities, components, engine := newWorld(t, 4*population, func(registrar *kernel.Registrar) {
			registrar.Subscribe[restructuringSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], we *WriteableEntities) {
				for e := range q.All() {
					visited++
					we.Despawn(e)
				}
	...
	t.Run("spawning one Entity per visited Entity", func(t *testing.T) {
		visited := 0
		entities, components, engine := newWorld(t, 4*population, func(registrar *kernel.Registrar) {
			registrar.Subscribe[restructuringSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], sp *Spawn[spawnSet]) {
				for range q.All() {
					visited++
					sp.New(spawnSet{Body: body{X: 1}, Velocity: typesVelocity{X: 1}})
				}
			}))
		})
		populate(entities, components, population)
...
func TestSpawnStagesItsComponentSetThroughAField(t *testing.T) {
	...
	round := func(create func(spawnSet) Entity) float64 {
		values := spawnSet{Body: body{X: 1}, Velocity: typesVelocity{X: 2}}
