...
func (f Feeder[E]) claim(system reflect.Type) {
	if f.feed == nil {
		panic(fmt.Sprintf("ecs: System %s is given a zero ecs.Feeder; build one with ecs.Feed", kernel.TypeName(system)))
	}
	if !f.value.Interface().(input).claim() {
		panic(fmt.Sprintf(
			...
			kernel.TypeName(system), kernel.TypeName(f.param)))
	}
}
