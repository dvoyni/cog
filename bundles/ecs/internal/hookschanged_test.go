...
func (p *changedOwnerPlugin) Dependencies() []kernel.PluginName {
	return []kernel.PluginName{typesName}
}
...
func newChangedWorld(t *testing.T, subscribe, own func(*kernel.Registrar)) (*Entities, *changedOwnerPlugin, *kernel.Engine) {
	...
	entities, _, engine := newWorldWith(t, 16, subscribe, []kernel.PluginName{typesName, "components", "changedowner"}, owner)
...
func (p *paddingPlugin) Dependencies() []kernel.PluginName { return []kernel.PluginName{typesName} }
...
func TestExplicitPaddingRecordsNoChangedForEqualFieldValues(t *testing.T) {
	...
	}, []kernel.PluginName{typesName, "components", "padding"}, owner)
