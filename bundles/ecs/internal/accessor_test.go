...
func TestUpdateForInsertsWhenAbsent(t *testing.T) {
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.Subscribe[accessorSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], colliders *Set[collider]) {
	...
	components.velocities.Set(bare, typesVelocity{})
...
func TestUpdateForIsSafeOnTheEntityBeingVisited(t *testing.T) {
	...
	entities, components, engine := newWorld(t, 8*population, func(registrar *kernel.Registrar) {
		registrar.Subscribe[accessorSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], bodies *Set[body], colliders *Set[collider]) {
			i := 0
			for e := range q.All() {
				visited++
...
func TestRemoveTakesAComponentAway(t *testing.T) {
	...
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.Subscribe[accessorSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], colliders *Remove[collider]) {
			for e := range q.All() {
				if !reported {
...
func TestAWritePointerIsInvalidatedByAGrowthDuringIteration(t *testing.T) {
	...
	run := func(ids uint32) (written int, grew bool) {
		...
		entities, components, engine := newWorld(t, ids, func(registrar *kernel.Registrar) {
			registrar.Subscribe[growingSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], bodies *Set[body]) {
				inserted := false
				for _, it := range q.All() {
					if !inserted {
