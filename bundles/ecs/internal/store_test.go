...
type typesPosition struct{ X, Y float32 }
...
func TestAStoreHoldsOneValuePerEntity(t *testing.T) {
	...
	store.Set(a, typesPosition{X: 1})
	store.Set(b, typesPosition{X: 2})
	if store.Len() != 2 {
...
func TestAStaleHandleFailsMembershipWithNothingElseConsulted(t *testing.T) {
	...
	store.Set(recycled, typesPosition{X: 2})
	if store.Has(e) {
...
func TestTheProbeIsTotal(t *testing.T) {
	entities := newEntities(8)
	store := NewStore[typesPosition](entities, 4)
	cases := []struct {
		name string
		e    Entity
	}{
...
func TestRemoveIsSwapRemoveAndThePopulationStaysExact(t *testing.T) {
	...
	for i := range live {
		...
		store.Set(live[i], typesPosition{X: float32(i)})
	}
...
func TestNothingShrinks(t *testing.T) {
	...
	store := NewStore[typesPosition](entities, 0)
	live := make([]Entity, 64)
	for i := range live {
		live[i] = entities.alloc()
		store.Set(live[i], typesPosition{X: float32(i)})
...
func TestTheReserveHintBuysAnAllocationFreeFill(t *testing.T) {
	...
	reservedStore := NewStore[typesPosition](entities, n)
	grownStore := NewStore[typesPosition](entities, 0)
	fill := func(store *Store[typesPosition]) func() {
		return func() {
			for _, e := range ids {
				store.Set(e, typesPosition{})
			}
		}
	}
...
func TestStoreCoreCarriesExactlyOneMethod(t *testing.T) {
	var _ storeCore = (*Store[typesPosition])(nil)
...
func filledStore(t *testing.T, n int) (*Store[typesPosition], *Entities) {
	t.Helper()
	entities := newEntities(uint32(n))
	store := NewStore[typesPosition](entities, uint32(n))
	for i := range n {
		store.Set(entities.alloc(), typesPosition{X: float32(i)})
	}
