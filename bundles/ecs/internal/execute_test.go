...
func TestASystemIsInvocableAsACommand(t *testing.T) {
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.HandleCommand[nudgeCmd](ToExecute[nudgeRequest, nudgeResponse](registrar, func(q *Query[typesMoveQuery], by *In[float32]) {
			amount := by.Get()
...
func TestASystemInvokedAsACommandMayNameItsRequest(t *testing.T) {
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.HandleCommand[nudgeCmd](ToExecute[nudgeRequest, nudgeResponse](registrar, func(request nudgeRequest, q *Query[typesMoveQuery]) {
			for _, it := range q.All() {
...
func TestASystemAnswersThroughItsResponseWrapper(t *testing.T) {
	entities, components, engine := newWorld(t, 64, func(registrar *kernel.Registrar) {
		registrar.HandleCommand[nudgeCmd](ToExecute[nudgeRequest, nudgeResponse](registrar, func(request nudgeRequest, q *Query[typesMoveQuery], answer *Resp[nudgeResponse]) {
			reply := nudgeResponse{}
			for _, it := range q.All() {
				it.Body.X += request.By
				reply.Moved++
...
func TestASubscriptionMayNotNameAResponse(t *testing.T) {
	message := composeAndFail(t, func(registrar *kernel.Registrar) {
		registrar.Subscribe[guardSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], answer *Resp[nudgeResponse]) {}))
	})
...
func TestAResponseOfTheWrongTypeIsRejected(t *testing.T) {
	defer func() {
		...
		message, _ := recovered.(string)
		for _, want := range []string{"ecs.Resp[ecs.body]", "ecs.nudgeResponse"} {
			if !strings.Contains(message, want) {
				t.Fatalf("panic %v does not name %q", recovered, want)
			}
		}
...
func TestASystemNamingTheResponseTwiceIsRejected(t *testing.T) {
	defer func() {
		...
		message, _ := recovered.(string)
		for _, want := range []string{"ecs.Resp[ecs.nudgeResponse]", "more than once"} {
			if !strings.Contains(message, want) {
				t.Fatalf("panic %v does not name %q", recovered, want)
			}
		}
