...
func subscribeNamedEvent(registrar *kernel.Registrar) {
	registrar.Subscribe[namedEventSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], tick app.UpdateEvent) {
		dt := float32(tick.Dt)
...
func subscribeFed(registrar *kernel.Registrar) {
	registrar.Subscribe[fedSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], in *In[float32]) {
		dt := in.Get()
		for _, it := range q.All() {
			it.Body.X += it.Velocity.X * dt
			it.Body.Y += it.Velocity.Y * dt
...
func subscribeUnhoisted(registrar *kernel.Registrar) {
	registrar.Subscribe[unhoistedSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], in *In[float32]) {
		for _, it := range q.All() {
			dt := in.Get()
			it.Body.X += it.Velocity.X * dt
			it.Body.Y += it.Velocity.Y * dt
...
func subscribeRecording(registrar *kernel.Registrar) {
	registrar.Subscribe[recordingSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], table *Read[*modelNames], out *Write[*drawLog]) {
...
func subscribePublishing(registrar *kernel.Registrar) {
	registrar.Subscribe[publishingSystem](ToHandler[app.UpdateEvent](registrar, func(handle kernel.Kernel, q *Query[typesMoveQuery]) {
		count := 0
		for _, it := range q.All() {
...
func boundInput(b *testing.B, n int) (*Query[typesMoveQuery], *In[float32]) {
	b.Helper()
	var query *Query[typesMoveQuery]
	var step *In[float32]
	entities, components, engine := newWorld(b, uint32(n), func(registrar *kernel.Registrar) {
		registrar.Subscribe[hoistSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], dt *In[float32]) { query, step = q, dt },
			Feed(func(e app.UpdateEvent) float32 { return float32(e.Dt) })))
	})
	populate(entities, components, n)
	frame(b, engine, 1)
...
func BenchmarkCommandSilent1k(b *testing.B) {
	benchmarkCommand(b, 1_000, func(request nudgeRequest, q *Query[typesMoveQuery]) {
		reply := nudgeResponse{}
		for _, it := range q.All() {
...
func BenchmarkCommandAnswering1k(b *testing.B) {
	benchmarkCommand(b, 1_000, func(request nudgeRequest, q *Query[typesMoveQuery], answer *Resp[nudgeResponse]) {
		reply := nudgeResponse{}
		for _, it := range q.All() {
			it.Body.X += request.By
			reply.Moved++
