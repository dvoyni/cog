...
func BenchmarkSpawnFourFields(b *testing.B) {
	...
	values := wideSet{Body: body{X: 1}, Velocity: typesVelocity{X: 2}, Collider: collider{Radius: 3}}
	benchmarkSpawn(b, writeable, func() Entity { return spawn.New(values) })
}
...
func BenchmarkSpawnTwoFieldsHandWritten(b *testing.B) {
	...
	benchmarkSpawn(b, writeable, func() Entity {
		...
		components.velocities.Set(e, typesVelocity{X: 2})
		return e
	})
}
...
type velocityQuery struct{ Velocity *typesVelocity }
...
func BenchmarkBarrierSpawning(b *testing.B) {
	benchmarkFrame(b, barrierEntities, subscribeThird(
		func(q *Query[velocityQuery], sp *Spawn[spawnSet], we *WriteableEntities) {
			walk(q)
			we.Despawn(sp.New(spawnSet{Body: body{X: 1}, Velocity: typesVelocity{X: 1}}))
		}))
}
	...
	measure := func(perTick int, ids uint32, frames int) float64 {
		...
		_, _, engine := newWorld(t, ids, func(registrar *kernel.Registrar) {
			registrar.Subscribe[typesChurnSystem](ToHandler[app.UpdateEvent](registrar, func(sp *Spawn[spawnSet], we *WriteableEntities) {
				...
				live = live[:0]
				for range perTick {
					live = append(live, sp.New(spawnSet{Body: body{X: 1}, Velocity: typesVelocity{X: 2}}))
				}
			}))
		})
