...
func TestStorableAdmitsImmutableIndirectionAndNothingElse(t *testing.T) {
	...
	legal := []struct {
		...
		{"everything the pointer-free rule already allowed", reflect.TypeFor[typesPosition]()},
...
func TestASetShowsInTheBytesOfTheComponentHoldingTheList(t *testing.T) {
	...
	entities, _, engine := newWorldWith(t, 16,
		...
		}, []kernel.PluginName{typesName, "components", "rich"}, rich)
...
func TestATrivialRowIsLeftWhereItLies(t *testing.T) {
	entities := newEntities(8)
	trivial := NewStore[typesPosition](entities, 8)
	if !trivial.trivial {
...
func (p *richPlugin) Dependencies() []kernel.PluginName { return []kernel.PluginName{typesName} }
