...
var (
	moveOffsets  = [2]uintptr{unsafe.Offsetof(typesMoveQuery{}.Body), unsafe.Offsetof(typesMoveQuery{}.Velocity)}
...
func fillMove(buffer *typesMoveQuery, b0, b1 unsafe.Pointer, r0, r1 uintptr) {
	*(*unsafe.Pointer)(unsafe.Pointer(&buffer.Body)) = unsafe.Add(b0, r0*unsafe.Sizeof(body{}))
	buffer.Velocity = *(*typesVelocity)(unsafe.Add(b1, r1*unsafe.Sizeof(typesVelocity{})))
}
...
func fillTyped(buffer *typedQuery, b0, b1 unsafe.Pointer, r0, r1 uintptr) {
	*(*unsafe.Pointer)(unsafe.Pointer(&buffer.W0)) = unsafe.Add(b0, r0*unsafe.Sizeof(w0{}))
	buffer.Label = *(*typesLabel)(unsafe.Add(b1, r1*unsafe.Sizeof(typesLabel{})))
}
...
func (moveFiller) fill(buffer *typesMoveQuery, b0, b1 unsafe.Pointer, r0, r1 uintptr) {
...
func allMoveValue(q *Query[typesMoveQuery]) iter.Seq2[Entity, typesMoveQuery] {
	return func(yield func(Entity, typesMoveQuery) bool) {
		var c [2]queryCursor
		...
		for row := len(walk) - 1; row >= 0; row-- {
			...
			if !yield(e, typesMoveQuery{
				Body:     (*body)(unsafe.Add(c0.rows, uintptr(row)*unsafe.Sizeof(body{}))),
				Velocity: *(*typesVelocity)(unsafe.Add(c1.rows, r1*unsafe.Sizeof(typesVelocity{}))),
			}) {
				return
			}
		}
	...
	return func(yield func(Entity, typedQuery) bool) {
		...
		for row := len(walk) - 1; row >= 0; row-- {
			...
			if !yield(e, typedQuery{
				...
				Label: *(*typesLabel)(unsafe.Add(c1.rows, r1*unsafe.Sizeof(typesLabel{}))),
			}) {
				return
			}
		}
...
func allMovePointer(q *Query[typesMoveQuery]) iter.Seq2[Entity, *typesMoveQuery] {
	return func(yield func(Entity, *typesMoveQuery) bool) {
		var c [2]queryCursor
		walk, driver := protoBind(q, moveOffsets[:], c[:])
		if validate || driver != 0 {
		...
		for row := len(walk) - 1; row >= 0; row-- {
			...
			*(*unsafe.Pointer)(unsafe.Pointer(&q.rows.Body)) = unsafe.Add(c0.rows, uintptr(row)*unsafe.Sizeof(body{}))
			q.rows.Velocity = *(*typesVelocity)(unsafe.Add(c1.rows, r1*unsafe.Sizeof(typesVelocity{})))
			if !yield(e, &q.rows) {
				return
			}
		}
	...
	return func(yield func(Entity, *typedQuery) bool) {
		...
		for row := len(walk) - 1; row >= 0; row-- {
			...
			*(*unsafe.Pointer)(unsafe.Pointer(&q.rows.W0)) = unsafe.Add(c0.rows, uintptr(row)*unsafe.Sizeof(w0{}))
			q.rows.Label = *(*typesLabel)(unsafe.Add(c1.rows, r1*unsafe.Sizeof(typesLabel{})))
			if !yield(e, &q.rows) {
				return
			}
		}
...
func movePlanned(q *Query[typesMoveQuery]) {
...
func moveGeneric(q *Query[typesMoveQuery]) {
	for _, it := range allGeneric2[typesMoveQuery, moveFiller](q, &moveOffsets) {
		it.Body.X += it.Velocity.X
		it.Body.Y += it.Velocity.Y
	}
}

func moveValue(q *Query[typesMoveQuery]) {
...
func movePointer(q *Query[typesMoveQuery]) {
