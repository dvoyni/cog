...
func TestErrUndeclaredDependencyStillFiresForADeferringSpawn(t *testing.T) {
	...
	err := compose([]kernel.PluginName{typesName})
	...
	if err := compose([]kernel.PluginName{typesName, "components"}); err != nil {
...
// here.
func deferringSpawnCycle(spawn *DeferredSpawn[spawnSet], we *WriteableEntities, despawn *DeferredDespawn, q *Query[typesMoveQuery]) {
	for e := range q.All() {
		despawn.Despawn(e)
		break
	}
...
func immediateSpawnCycle(spawn *Spawn[spawnSet], we *WriteableEntities, despawn *DeferredDespawn, q *Query[typesMoveQuery]) {
	for e := range q.All() {
		despawn.Despawn(e)
		break
	}
