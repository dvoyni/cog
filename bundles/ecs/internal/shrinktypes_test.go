...
func spiked(t *testing.T) *spikedWorld {
	...
	w.entities, w.components, w.engine = newWorld(t, 16, func(registrar *kernel.Registrar) {
		registrar.Subscribe[typesMoveSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery]) {
			w.query = q
			move(q)
		}))
	})
	...
	for i := range spike {
		...
		w.components.velocities.Set(spike[i], typesVelocity{Y: 1})
	}
	frame(t, w.engine, 1)
	for i, e := range spike {
		if spikeSurvivor(i) {
...
func TestAHandleToADroppedIndexNeverMatchesItsNextEntity(t *testing.T) {
	...
	for round := range 4 {
		...
		next := entities.alloc()
		store.Set(next, typesPosition{X: float32(round)})
