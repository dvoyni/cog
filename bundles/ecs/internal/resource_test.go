...
var boundDeps = []kernel.PluginName{typesName, "components", "binding"}
...
func TestASystemReachesAnotherPluginsResourceThroughItsSignature(t *testing.T) {
	...
	entities, components, engine := newWorldWith(t, 64,
		func(registrar *kernel.Registrar) {
			registrar.Subscribe[recordSystem](ToHandler[app.UpdateEvent](registrar, func(q *Query[typesMoveQuery], table *Read[*modelNames], out *Write[*drawLog]) {
