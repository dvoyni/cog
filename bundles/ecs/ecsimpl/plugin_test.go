...
// subscribes its Systems through ecs's contract root, and declares a dependency
...
func TestNewPublishesTheAuthority(t *testing.T) {
	game := &gamePlugin{}
	engine := kernel.New(map[kernel.PluginName]any{ecs.Name: Config{PrewarmEntities: 16}}).
...
func TestAMissingAuthorityFailsComposition(t *testing.T) {
	...
	if failure == nil {
		t.Fatalf("a game composed without ecsimpl.New() composed")
	}
}
