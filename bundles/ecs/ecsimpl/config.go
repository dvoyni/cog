...
// Config is the ecs plugin's configuration, keyed by ecs.Name in the engine's
// configuration map. A zero field takes its default, so a caller names only
// what it changes:
//
//	kernel.New(map[kernel.PluginName]any{ecs.Name: ecsimpl.Config{PrewarmEntities: 50_000}})
type Config struct {
