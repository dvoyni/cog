...
`github.com/dvoyni/cog/bundles/input` gains a way to play a scripted sequence of input
...
[bundles/input/docs/specs/mcp.md](./mcp.md), and they are a one-line adapter over
`Play`.
...
- **`input.ApplyCmd` is already the right shape.** `ApplyRequest{Changes []Change}`
  folds a batch under one write lock (`bundles/input/commands.go:7-12`) and publishes
  one discrete event per change (`bundles/input/commandsimpl.go:7-19`).
- **`Change` cannot cross a protocol.** Every field is unexported
  (`bundles/input/contract.go:205-213`), so `ApplyRequest` reflects to a schema with no
...
- **Mouse buttons *are* keys.** `input.Key` unifies them "so a single
  ...
  (`bundles/input/contract.go:18-22`). There is no separate mouse verb to design.
...
- **A same-tick press is invisible to `Pressed`.** `state.apply` adds then
  ...
  (`bundles/input/state.go:38-52`). `JustPressed` and `JustReleased` both fire,
...
- **…but a same-tick click already works for `ui`.** `processInteractions`
  ...
  `InteractionClick` (`bundles/ui/layout.go:876-897`). The pointer is live, so one batch
  is a complete click.
...
(`bundles/input/state.go:65`) promotes `pend*` into `just*` only on a tick. This is
...
already treats it (`bundles/input/state.go:45`). Rejecting it was considered — it
...
(`extensions/gogpu/internal/input.go:14-26`), so a faithful `text` step would emit both. **It does
not.**
...
passed through (`extensions/gogpu/internal/input.go:48`), nothing in cog consumes it, and cog has no
unit to promise.
