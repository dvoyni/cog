...
func TestPackInstanceSkipsAnimationAndSkinningForABufferBuiltDraw(t *testing.T) {
	instance := packInstance(m.Mat4{}, types.AnimBinding{Offset: types.SceneNoAnim}, 0)
	if instance.AnimOffset != types.SceneNoAnim {
		t.Fatalf("animOffset is %d, want SCENE_NO_ANIM (%d)", instance.AnimOffset, types.SceneNoAnim)
	}
...
func TestPackInstanceFlagsNonUniformScaleOnly(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			instance := packInstance(test.matrix, types.AnimBinding{Offset: types.SceneNoAnim}, 0)
...
func TestPackInstanceRowsTransformLikeTheMatrix(t *testing.T) {
	...
	instance := packInstance(world, types.AnimBinding{Offset: types.SceneNoAnim}, 0)
