...
func passAspect(id scene.CameraID, pass scene.Pass, view *gfx.Viewport) (float32, error) {
	if pass.Target.IsNone() {
		if _, clears := pass.ClearColor.Get(); clears {
			return 0, scene.ErrColourlessPassClearsColour{Camera: id, Tag: types.PassTagOf(pass)}
		}
		...
		if !ok {
			return 0, scene.ErrColourlessPassWithoutDepth{Camera: id, Tag: types.PassTagOf(pass)}
		}
...
func aspectOf(id scene.CameraID, pass scene.Pass, width, height float32) (float32, error) {
	if width <= 0 || height <= 0 {
		return 0, scene.ErrPassTargetUnsized{Camera: id, Tag: types.PassTagOf(pass)}
	}
