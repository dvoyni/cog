...
// everything scene subscribes or handles, the load System and the debug
...
func TestTheLoadSystemIsTheOnlyOneWritingTheLookup(t *testing.T) {
	...
	if loader == nil || recorder == nil {
		t.Fatal("the description does not list both of scene's Systems")
	}
	...
	if len(debug) != len(writers)-1 || len(debug) != 10 {
		t.Errorf("scene's handlers writing *model.Lookup are %v, want the load System and the ten debug Systems", writers)
	}
