...
func (e ErrCameraAlreadyRecorded) Error() string {
	return fmt.Sprintf("scene: camera %d is held by two Entities this frame; the first wins", e.Camera)
}
...
// ErrViewportUnsized reports a ViewProjection asked for a viewport with a
// side of zero or less, which has no aspect to project through.
type ErrViewportUnsized struct {
	Camera   CameraID
	Viewport m.Vec2
}

func (e ErrViewportUnsized) Error() string {
	return fmt.Sprintf("scene: camera %d has no projection for a %vx%v viewport", e.Camera, e.Viewport.X, e.Viewport.Y)
}
