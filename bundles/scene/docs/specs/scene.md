...
| gogpu's unvalidated `arrayStride` | **closed**, fixed: cog pins the fork `dvoyni/wgpu` by `replace`, whose pipeline validation rejects a zero, misaligned or over-limit stride and an attribute past its stride on every native backend instead of letting each diverge. Stride 0 is rejected, not emulated. The same patch is bound for upstream | [47](https://github.com/dvoyni/cog/issues/47) |
