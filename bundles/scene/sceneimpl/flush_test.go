...
func TestNeverCullAndExplicitBoundsOverrideTheBakedSphere(t *testing.T) {
	...
	h := newHarnessWithErrors(t, func(q *scene.OpQueue) {
		...
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, 5), Color: testBoxColor, NeverCull: true})
		// Behind the camera by its mesh, but its explicit sphere reaches the
		// camera.
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, 5), Color: testBoxColor, Bounds: m.Sphere{Radius: 10}})
		// Behind the camera by its mesh, and its explicit sphere says so too.
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, 5), Color: testBoxColor, Bounds: m.Sphere{Radius: 1}})
	}, &reported)
	h.frame()
...
func TestOpaqueDrawsGroupByMaterialNotRecordingOrder(t *testing.T) {
	h := newHarness(t, func(q *scene.OpQueue) {
		...
		for i, material := range []scene.Material{b, a, b, a, b} {
			internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(float32(i), 0, -5), Color: testBoxColor, Material: material})
		}
	})
	h.frame()
...
func TestBlendDrawsFollowOpaqueAndSortBackToFront(t *testing.T) {
	h := newHarness(t, func(q *scene.OpQueue) {
		...
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, -2), Color: testBoxColor, Material: blendMaterial(1)})
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, -20), Color: testBoxColor, Material: blendMaterial(2)})
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, -50), Color: testBoxColor, Material: opaqueMaterial(3)})
	})
	h.frame()
...
func TestASeventeenthLightIsDroppedWithoutAReport(t *testing.T) {
	...
	if pass := h.passes()[0]; pass.Lights != internal.MaxLights {
		t.Errorf("the pass packed %d lights, want the cap of %d", pass.Lights, internal.MaxLights)
	}
	if len(reported) != 0 {
...
func TestAMultiTagMaterialDrawsInEveryPassItServes(t *testing.T) {
	...
	h := newHarness(t, func(q *scene.OpQueue) {
		...
		q.Camera(testCamera, descr)
		internal.OpQueueDraw(q, internal.DrawRecord{Shape: internal.ShapeBox, Transform: scene.At(0, 0, -5), Color: testBoxColor, Material: both})
