...
func TestALineIsAStretchedBoxFromStartToEnd(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if len(internal.OpQueueRecordedDraws(&q)) != 1 {
				t.Fatalf("recorded %d draws, want 1", len(internal.OpQueueRecordedDraws(&q)))
			}
			record := internal.OpQueueRecordedDraws(&q)[0]
			...
			if packInstance(world, internal.AnimBinding{Offset: internal.SceneNoAnim}, 0).Flags&sceneNonUniform == 0 {
...
func TestLinesAreSelfLitAndSolidsAreLit(t *testing.T) {
	...
	for i, record := range internal.OpQueueRecordedDraws(&q) {
...
func TestOnlyStretchedShapesFlagNonUniform(t *testing.T) {
	...
	sphere, plane := internal.OpQueueRecordedDraws(&q)[0], internal.OpQueueRecordedDraws(&q)[1]

	if flags := packInstance(sphere.World(), internal.AnimBinding{Offset: internal.SceneNoAnim}, 0).Flags; flags&sceneNonUniform != 0 {
	...
	if flags := packInstance(plane.World(), internal.AnimBinding{Offset: internal.SceneNoAnim}, 0).Flags; flags&sceneNonUniform == 0 {
...
func TestWireBoxEdgesCloseAtTheCorners(t *testing.T) {
	...
	if len(internal.OpQueueRecordedDraws(&q)) != 12 {
		t.Fatalf("recorded %d draws, want 12", len(internal.OpQueueRecordedDraws(&q)))
	}
	extent := m.Box3{Min: center, Max: center}
	for _, record := range internal.OpQueueRecordedDraws(&q) {
		world := record.World()
