...
const morphModelWords = internal.MorphRangeWords + 2*internal.MorphTargetHeaderWords + 2*2
...
func decodeAnimBlock(t *testing.T, arena []byte, offset uint32) animBlock {
	...
	list := base + internal.AnimHeaderVec4s*16 + int(block.playCount)*16
	for i := range int(block.targetCount) {
		at := list + i*8
		block.targets = append(block.targets, internal.SceneMorphWeight{
			Target: word(at),
...
func TestAMorphedModelBindsItsOwnDeltasAndABoxBindsNone(t *testing.T) {
	...
	if got, want := len(boundBytes(t, h, "sceneMorphDeltas")), morphModelWords*internal.MorphWordSize; got != want {
...
func TestAnEmptyMorphWeightsIsAnOverrideRatherThanAnAbsence(t *testing.T) {
	...
	if got := firstInstance(t, h).AnimOffset; got != internal.SceneNoAnim {
...
func TestTheLookupReportsMorphTargetsAndBytes(t *testing.T) {
	...
	if want := morphModelWords * internal.MorphWordSize; bytes != want {
...
func TestASkinnedAndMorphedModelBindsBothOfItsOwnBuffers(t *testing.T) {
	...
	if got, want := len(boundBytes(t, h, "scenePoses")), 62*internal.PoseSize; got != want {
	...
	if got, want := len(boundBytes(t, h, "sceneMorphDeltas")),
		(internal.MorphRangeWords+internal.MorphTargetHeaderWords+2)*internal.MorphWordSize; got != want {
		t.Errorf("the bound delta buffer is %d bytes, want the model's %d", got, want)
	}
	block := decodeAnimBlock(t, boundBytes(t, h, "sceneAnim"), instance.AnimOffset)
