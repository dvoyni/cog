...
func readMeshRecord(data []byte, index int) internal.SceneMesh {
	at := data[index*meshRecordSize:]
	read := func(offset int) m.Vec2 {
		return m.Vec2{X: readFloat32(at[offset:]), Y: readFloat32(at[offset+4:])}
	}
	return internal.SceneMesh{UV0Scale: read(0), UV0Bias: read(8), UV1Scale: read(16), UV1Bias: read(24)}
}
...
func TestAMeshWithAUVRangeNamesItsOwnSlotAfterTheIdentity(t *testing.T) {
	...
	if slot0 := readMeshRecord(meshes, 0); slot0 != internal.IdentityMesh {
		t.Fatalf("slot 0 is %+v, want the identity %+v", slot0, internal.IdentityMesh)
	}
	_, _, want := internal.PackVertices(new([]byte), uvTriangle())
