...
func TestLightRecordIsFortyEightBytesAndTheFrameBlockHoldsSixteen(t *testing.T) {
	...
	if len(block.Lights) != internal.MaxLights || internal.MaxLights != 16 {
...
func TestAPointLightPacksAZeroDirectionAndAUnitCone(t *testing.T) {
	light, err := packLight(scene.LightDescr{
...
func TestALightPremultipliesIntensityAndStoresInverseRangeToTheFourth(t *testing.T) {
	light, err := packLight(scene.LightDescr{
		Color: m.NewColorLinear(1, 1, 1, 1), Intensity: 3, Range: 2, Kind: scene.LightPoint,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestASpotLightPacksTheConeAsScaleAndOffset(t *testing.T) {
	...
	light, err := packLight(scene.LightDescr{
		...
		Color: m.NewColorLinear(1, 1, 1, 1), Kind: scene.LightSpot,
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestAZeroOuterConeMeansAQuarterPi(t *testing.T) {
	light, err := packLight(scene.LightDescr{Direction: m.Vec3{Z: -1}, Kind: scene.LightSpot})
	if err != nil {
		t.Fatal(err)
	}
...
func TestADegenerateSpotLightIsReportedAndSkipped(t *testing.T) {
	_, err := packLight(scene.LightDescr{Direction: m.Vec3{Z: -1}, InnerCone: 0.7, OuterCone: 0.5, Kind: scene.LightSpot})
	var inverted scene.ErrSpotConeInverted
	...
	_, err = packLight(scene.LightDescr{Direction: m.Vec3{Z: -1}, InnerCone: 0.5, OuterCone: 0.5, Kind: scene.LightSpot})
	if !errors.As(err, &inverted) {
		t.Errorf("an inner cone equal to the outer reported %v, want ErrSpotConeInverted", err)
	}
	_, err = packLight(scene.LightDescr{Kind: scene.LightSpot})
	var missing scene.ErrSpotDirectionMissing
...
func TestContributionIsTheFalloffAtTheEyeTimesLuminance(t *testing.T) {
	white, _ := packLight(scene.LightDescr{Position: m.Vec3{Z: -2}, Range: 4, Color: m.NewColorLinear(1, 1, 1, 1), Kind: scene.LightPoint})
	eye := m.Vec3{}
	...
	green, _ := packLight(scene.LightDescr{Position: m.Vec3{Z: -2}, Color: m.NewColorLinear(0, 1, 0, 1), Kind: scene.LightPoint})
	red, _ := packLight(scene.LightDescr{Position: m.Vec3{Z: -2}, Color: m.NewColorLinear(1, 0, 0, 1), Kind: scene.LightPoint})
	...
	away, _ := packLight(scene.LightDescr{Position: m.Vec3{Z: -2}, Direction: m.Vec3{Z: -1}, Color: m.NewColorLinear(1, 1, 1, 1), Kind: scene.LightSpot})
...
func TestTheCapKeepsTheBrightestSixteenNotTheFirstSixteen(t *testing.T) {
	...
	for _, order := range [][]preparedLight{nearestFirst, farthestFirst} {
		...
		if selection.count != internal.MaxLights {
			t.Fatalf("kept %d lights, want the cap of %d", selection.count, internal.MaxLights)
		}
...
func TestALightLayerMaskSelectsCameras(t *testing.T) {
	...
	var selection lightSelection
	selection.selectLights(forwardFrustum(), m.Vec3{}, scene.Layer(2), lights)
...
func TestPointLightAndSpotLightRecordTheirKind(t *testing.T) {
	...
	lights := internal.OpQueueFlushLights(&q)
