...
var (
	...
	// temporary mesh.
	noSphere = internal.MeshRecord{}
	// boxSphere is a mesh with a baked sphere, the unit box's.
	boxSphere = internal.MeshRecord{Bounds: m.Sphere{Radius: float32(math.Sqrt(3)) / 2}}
)
...
func TestWorldRadiusIsTheLocalRadiusTimesTheLargestAxisScale(t *testing.T) {
	scaled := prepareDraw(internal.DrawRecord{Transform: scene.At(1, 2, 3).WithScale(2)}, boxSphere)
	if !scaled.cullable {
	...
	stretchedDraw := prepareDraw(internal.DrawRecord{Transform: scene.Transform{Scale: m.Vec3{X: 1, Y: 5, Z: 2}}}, boxSphere)
...
func TestACullCountsOnlyTheLayersTheCameraSees(t *testing.T) {
	draws := []internal.DrawRecord{{Layers: scene.Layer(1)}, {Layers: scene.Layer(2)}}
	prepared := []preparedDraw{
		{sphere: m.Sphere{Center: m.Vec3{Z: -5}, Radius: 1}, cullable: true},
		{sphere: m.Sphere{Center: m.Vec3{Z: -5}, Radius: 1}, cullable: true},
	}
...
func TestASteadyCullAllocatesNothing(t *testing.T) {
	draws := make([]internal.DrawRecord, 64)
