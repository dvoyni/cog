...
func morphAnim() *internal.ResidentAnimation {
...
func TestBlendMorphWeightsFallsBackToTheRestRow(t *testing.T) {
	...
	got := internal.BlendMorphWeights(morphAnim(), "m.glb", nil, nil, nil, false, nil, report)
...
func TestBlendMorphWeightsOverridesWholesale(t *testing.T) {
	report, keys := collectReports()
	anim := morphAnim()
	plays, frames := internal.ResolvePlays(anim, "m.glb", []scene.ClipPlay{
		{Clip: "smile", Weight: 1},
	}, nil, nil, report)
	got := internal.BlendMorphWeights(anim, "m.glb", plays, frames, []float32{0.75}, true, nil, report)
...
func TestSelectMorphTargetsCullsByMagnitude(t *testing.T) {
	...
	got := internal.SelectMorphTargets([]float32{0, 0.5, 1e-6, -0.25}, nil, "m.glb", report)
	if len(got) != 2 {
		t.Fatalf("kept %v, want the two targets above the tolerance", got)
	}
	if got[0] != (internal.SceneMorphWeight{Target: 1, Weight: 0.5}) {
		t.Errorf("entry 0 = %+v, want target 1 at 0.5", got[0])
	}
	if got[1] != (internal.SceneMorphWeight{Target: 3, Weight: -0.25}) {
...
func TestPackAnimLaysTheMorphListOutInWholeVec4s(t *testing.T) {
	...
	block := morphBlock{
		binding: internal.MorphBinding{Base: 7, Stride: 2, Targets: 3},
		targets: []internal.SceneMorphWeight{{Target: 0, Weight: 1}, {Target: 2, Weight: 0.5}},
	}
	...
	// header plus one.
	if want := uint32(internal.AnimHeaderVec4s + 1); build.packAnim(nil, block) != want {
		t.Errorf("the second block is not at %d", want)
	}
	// An odd count pads: three entries are two vec4s, the last half empty.
	odd := morphBlock{
		binding: block.binding,
		targets: append(block.targets, internal.SceneMorphWeight{Target: 1, Weight: 0.25}),
	}
	third := build.packAnim(nil, odd)
	if want := uint32(2 * (internal.AnimHeaderVec4s + 1)); third != want {
		t.Errorf("the third block is at %d, want %d", third, want)
	}
	// Two blocks of header plus one vec4, then one of header plus two.
	if got, want := len(build.anims.bytes()), (2*(internal.AnimHeaderVec4s+1)+internal.AnimHeaderVec4s+2)*16; got != want {
