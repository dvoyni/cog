...
func TestTheBundledShaderIsMountedForTheFirstFrame(t *testing.T) {
	h := newHarness(t, func(q *scene.OpQueue) {})
	embedded, err := shaderFS.ReadFile(internal.SceneShaderPath)
	if err != nil {
		t.Fatalf("read the embedded shader: %v", err)
	}
	mounted := h.readFile(t, internal.SceneShaderPath)
...
func TestTheMaterialRecordIsBoundAsItsOwnRange(t *testing.T) {
	...
	if materials[0].size != int(unsafe.Sizeof(internal.ScenePbrRecord{})) {
		t.Fatalf("the material range is %d bytes, want the %d-byte record",
			materials[0].size, unsafe.Sizeof(internal.ScenePbrRecord{}))
	}
}
