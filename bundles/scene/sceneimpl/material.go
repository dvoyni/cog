...
func materialKeyOf(material scene.Material) materialKey {
	...
	for i := range material {
		h.WriteString(string(internal.MaterialTagOf(material[i])))
...
func (t *materialTable) reset(bundled [internal.VariantCount]scene.Material) {
	if t.keys == nil {
...
func (t *materialTable) intern(report func(error), material scene.Material, variant internal.ShaderVariant) int32 {
	if material == nil {
...
func (t *materialTable) add(report func(error), material scene.Material) int32 {
	...
	for i := range material {
		t.internTag(internal.MaterialTagOf(material[i]))
	}
	...
	for i := range material {
		tag := t.internTag(internal.MaterialTagOf(material[i]))
		if record.entries[tag] != noEntry {
			report(scene.ErrMaterialTagAlreadyServed{Tag: internal.MaterialTagOf(material[i])})
			continue
		}
