...
type plugin struct {
	...
	temporaries []internal.MeshRecord
	...
	// frames.
	modelViews []internal.ModelView
	modelAnims []internal.AnimBinding
	modelPlays []internal.ScenePlayRecord
	...
	modelWeightFrames []internal.WeightFrames
	modelWeights      []float32
	modelTargets      []internal.SceneMorphWeight
...
func (p *plugin) Register(registrar *kernel.Registrar, value any) error {
	...
	registrar.HandleCommand[internal.LoadModelCmd](loadModelCmdImpl)
...
func (p *plugin) flushFrame(
	...
	cameras := internal.OpQueueBeginFlush(write)
	defer internal.OpQueueEndFlush(write)
	for _, id := range internal.OpQueueDuplicates(write) {
	...
	internal.LookupApplyUnloads(lookup, gfxResources.ReleaseTexture)
	...
	internal.LookupDrainMeshes(lookup, internal.MeshBaker{
	...
	p.materials.reset(internal.LookupEnsureBundled(lookup, bakeTexture))
	...
	p.prepareDraws(report, lookup, write, bake, internal.OpQueueFlushDraws(write))
	p.preparedLights = prepareLights(report, p.preparedLights, internal.OpQueueFlushLights(write))
	for i := range cameras {
...
func (p *plugin) prepareDraws(
	...
	bake internal.BakeFunc, draws []internal.DrawRecord,
) {
	...
	for i := range draws {
		record := &draws[i]
		ref := record.Mesh
		if record.Shape != internal.ShapeNone {
			ref = internal.LookupEnsureUnit(lookup, record.Shape, bake)
		}
		...
		case !ok:
			...
			if internal.MeshRefSource(ref) != internal.MeshNone {
		...
		case !mesh.Standard && record.Material == nil:
			...
			ref, mesh = scene.MeshRef{}, internal.MeshRecord{}
		}
		...
		p.prepared[i].interned = p.materials.intern(report, record.Material,
			internal.VariantFor(skin.Bound, skin.Morphed))
		if !skin.Bound && !skin.Morphed {
			p.prepared[i].anim.Offset = internal.SceneNoAnim
		}
	}
}
...
) (internal.MeshRecord, bool) {
	if internal.MeshRefSource(ref) == internal.MeshTemporary {
		id := internal.MeshRefIndex(ref)
		if internal.MeshRefGeneration(ref) != internal.OpQueuePublishedFrame(write) || id == 0 || int(id) > len(p.temporaries) {
			return internal.MeshRecord{}, false
		}
		return p.temporaries[id-1], true
	}
	return internal.LookupMesh(lookup, ref)
}
...
func (p *plugin) buildTemporaries(report func(error), write *scene.OpQueue) {
	recording := internal.OpQueueFlushMeshes(write)
...
func (p *plugin) flushCamera(
	k kernel.Kernel, write *scene.OpQueue, lookup *scene.Lookup, view *gfx.Viewport, camera internal.CameraRecord,
) {
	...
	viewMatrix, ok := internal.CameraView(camera.Descr.Transform)
	if !ok {
	...
	if len(passes) == 0 {
		p.defaultPasses[0] = internal.DefaultPass()
...
func (p *plugin) flushPass(
	...
	camera internal.CameraRecord, viewMatrix m.Mat4, pass scene.Pass,
) {
	...
	projectionMatrix, err := internal.Projection(camera.ID, camera.Descr, aspect)
	if err != nil {
		k.ReportError(err)
		return
	}
	...
	draws := internal.OpQueueFlushDraws(write)
	...
	block := packFrameLighting(sceneFrameBlock{
		...
		CameraPosition: eye,
		ViewDirection:  internal.ViewDirection(camera.Descr),
	...
	tag := p.materials.internTag(internal.PassTagOf(pass))
	...
	internal.OpQueuePublishPass(write, result)
	internal.OpQueuePublishBatches(write, p.build.batches)
}
...
func groupRun(entries []sortEntry, survivors []survivor, draws []internal.DrawRecord) int {
