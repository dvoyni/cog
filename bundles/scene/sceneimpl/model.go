...
func (p *plugin) expandModels(
	...
	models := internal.OpQueueFlushModels(write)
	if len(models) == 0 {
		return
	}
	...
	worlds := 0
	for i := range models {
		p.modelViews[i] = internal.ModelView{}
		entry, ok := internal.LookupRequestModel(lookup, k, models[i].Path)
		if !ok {
			continue
		}
		view, err := entry.View(models[i].Path, models[i].Scene, models[i].Node)
		if err != nil {
			internal.LookupReportOnce(lookup, report, internal.SelectorReportKey(err), err)
			continue
		}
		p.modelViews[i] = view
	...
	for i := range models {
		...
		for j := range view.Primitives {
			...
			material, record := owned.Variants[internal.VariantFor(anim.Skin.Bound, anim.Skin.Morphed)], &owned.Record
			...
			group := uint32(0)
			if len(instances) > 1 {
				group = uint32(internal.OpQueueDrawCount(write)) + 1
			}
			for _, instance := range instances {
				...
				internal.OpQueueAppendDraw(write, internal.DrawRecord{
					Layers:   model.Layers,
...
func (p *plugin) resolveModelAnimation(
	report func(error), lookup *scene.Lookup, models []internal.ModelDrawRecord,
) {
	p.modelAnims = grow(p.modelAnims, len(models))
	p.modelMorphOffsets = p.modelMorphOffsets[:0]
	once := func(key string, err error) { internal.LookupReportOnce(lookup, report, key, err) }
	for i := range models {
		p.modelAnims[i] = internal.AnimBinding{Offset: internal.SceneNoAnim, MorphAt: -1}
		view := &p.modelViews[i]
		if !view.Resolved {
			continue
		}
		...
		p.modelPlays, p.modelWeightFrames = internal.ResolvePlays(
		...
		if view.Rerooted && view.RerootJoint >= 0 {
			if pose, ok := internal.BlendJoint(anim, p.modelPlays, view.RerootJoint); ok {
...
func (p *plugin) packModelMorphs(
	binding *internal.AnimBinding, anim *internal.ResidentAnimation, view *internal.ModelView,
	model *internal.ModelDrawRecord, report internal.ReportOnce,
) {
	p.modelWeights = internal.BlendMorphWeights(
	...
	for j := range view.Primitives {
		...
		if morph.Morphed() {
			...
			block.targets = internal.SelectMorphTargets(slots, p.modelTargets[:0], model.Path, report)
