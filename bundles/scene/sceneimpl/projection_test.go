...
func TestFovYIsTheLiteralVerticalFieldOfView(t *testing.T) {
	...
	projection, err := internal.Projection(1, descr, 2)
	if err != nil {
...
func TestDepthIsConventional(t *testing.T) {
	...
	projection, err := internal.Projection(1, scene.CameraDescr{FovY: 1, Near: 1, Far: 100}, 1)
	if err != nil {
		t.Fatalf("projection: %v", err)
	}
...
func TestADegenerateProjectionIsReported(t *testing.T) {
	...
	for _, test := range cases {
		if _, err := internal.Projection(1, test.descr, 1); err == nil {
...
func TestObliqueWithoutShearIsOrthographic(t *testing.T) {
	...
	oblique, err := internal.Projection(1, scene.CameraDescr{Projection: scene.Oblique, Height: 10, Near: 1, Far: 100}, 2)
	if err != nil {
		t.Fatalf("oblique projection: %v", err)
	}
	orthographic, err := internal.Projection(1, scene.CameraDescr{Projection: scene.Orthographic, Height: 10, Near: 1, Far: 100}, 2)
	if err != nil {
...
func TestObliqueRequiresAHeightAndNothingElse(t *testing.T) {
	...
	if _, err := internal.Projection(1, scene.CameraDescr{Projection: scene.Oblique, Shear: 1, Near: 1, Far: 10}, 1); err == nil {
	...
	for _, shear := range []float32{0, -0.5, 1000} {
		...
		if _, err := internal.Projection(1, descr, 1); err != nil {
...
func TestShearIsReadByObliqueAlone(t *testing.T) {
	...
	for _, kind := range []scene.ProjectionKind{scene.Perspective, scene.Orthographic} {
		...
		plain, err := internal.Projection(1, descr, 2)
		if err != nil {
			t.Fatalf("projection: %v", err)
		}
		descr.Shear = 0.75
		sheared, err := internal.Projection(1, descr, 2)
		if err != nil {
			t.Fatalf("projection: %v", err)
		}
		if plain != sheared {
...
func TestPerspectiveDefersItsViewDirectionToTheFragment(t *testing.T) {
	...
	direction := internal.ViewDirection(scene.CameraDescr{FovY: 1, Near: 1, Far: 100})
	if direction.W != 0 {
...
func TestOrthographicViewDirectionIsTheCameraAxis(t *testing.T) {
	...
	direction := internal.ViewDirection(scene.CameraDescr{Projection: scene.Orthographic, Height: 10, Near: 1, Far: 100})
	if direction.W != 1 {
...
func TestObliqueViewDirectionIsTheProjectionRayNotTheCameraAxis(t *testing.T) {
	...
	direction := internal.ViewDirection(descr)
	if direction.W != 1 {
	...
	axis := internal.ViewDirection(scene.CameraDescr{Transform: descr.Transform, Projection: scene.Orthographic, Height: 10, Near: 1, Far: 100})
...
func TestViewDirectionIsUnitLengthUnderAScaledCamera(t *testing.T) {
	...
	if got := internal.ViewDirection(scaled).Vec3().Length(); !near(got, 1) {
		t.Errorf("view direction length under a scaled camera = %v, want 1", got)
	}
	stretched := scaled
	stretched.Transform.Scale = m.Vec3{X: 2, Y: 3, Z: 4}
	if got := internal.ViewDirection(stretched).Vec3().Length(); !near(got, 1) {
...
func TestTheViewDirectionSelectorPicksRadialOrConstant(t *testing.T) {
	...
	perspective := sceneFrameBlock{
		...
		ViewDirection:  internal.ViewDirection(scene.CameraDescr{Transform: transform, FovY: 1, Near: 1, Far: 100}),
	}
	...
	oblique := sceneFrameBlock{CameraPosition: cameraPosition(transform), ViewDirection: internal.ViewDirection(descr)}
	want := internal.ViewDirection(descr).Vec3()
	for _, surface := range surfaces {
