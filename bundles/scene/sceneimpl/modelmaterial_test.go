...
func residentModelDraw(t testing.TB, doc *gltf.Document, draw scene.ModelDraw) (*harness, []internal.DrawRecord) {
	t.Helper()
	...
	var records []internal.DrawRecord
	h.inspect(func(q *scene.OpQueue) { records = append(records, internal.OpQueueFlushDraws(q)...) })
	return h, records
}
...
func TestAPlainModelDrawBindsTheFilesRecordsWithNoCopy(t *testing.T) {
	...
	h.kernel.ExecuteCommand[lookupProbeCmd](lookupProbeRequest{lookup: func(lookup *scene.Lookup) {
		key, _ := internal.ModelKey(modelPath)
		entry := internal.LookupModelEntry(lookup, key)
		...
		for i := range records {
			...
			if &records[i].Material[0] != &owned.Variants[internal.VariantStatic][0] {
...
func TestOverrideParamsKeepTheFilesTexturesAndReachTheDrawsParameters(t *testing.T) {
	...
	h.kernel.ExecuteCommand[lookupProbeCmd](lookupProbeRequest{lookup: func(lookup *scene.Lookup) {
		...
		entry := internal.LookupModelEntry(lookup, key)
		for i := range records {
			if &records[i].Material[0] != &entry.Materials[i].Variants[internal.VariantStatic][0] {
...
func TestAModelDrawWithAMaterialReplacesTheFilesWholesale(t *testing.T) {
	replacement := scene.Material{{Descr: gfx.MaterialWithState(
		gfx.ShaderWithResource(internal.SceneShaderPath), internal.PbrState(internal.AlphaOpaque, false))}}
