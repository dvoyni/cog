...
func prepareLights(report func(error), dst []preparedLight, lights []internal.LightRecord) []preparedLight {
	dst = dst[:0]
	for i := range lights {
...
type lightSelection struct {
	lights [internal.MaxLights]sceneLight
	scores [internal.MaxLights]float32
	count  int
}
...
func (s *lightSelection) selectLights(
	...
	for i := range lights {
		light := &lights[i]
		if !internal.LayerMaskDrawnBy(light.layers, cullMask) {
			continue
		}
...
func (s *lightSelection) offer(light *sceneLight, score float32) {
	if s.count < internal.MaxLights {
		s.lights[s.count], s.scores[s.count] = *light, score
		s.count++
		return
	}
	weakest := 0
	for i := 1; i < internal.MaxLights; i++ {
