...
func TestATemporaryMeshDrawsWithoutBaking(t *testing.T) {
	h := newHarness(t, func(q *scene.OpQueue) {
		...
		if ref.ID()&internal.TemporaryMeshID == 0 {
...
func TestAStaleRefDoesNotDrawTheMeshThatReusedItsSlot(t *testing.T) {
	...
	if internal.MeshRefIndex(reused) != internal.MeshRefIndex(stale) {
		t.Fatalf("the released slot %d was not reissued, got %d", internal.MeshRefIndex(stale), internal.MeshRefIndex(reused))
	}
	*h.reported = (*h.reported)[:0]
	h.frame()
...
func TestATemporaryRefFromAnEarlierFrameIsReportedAndSkipped(t *testing.T) {
	...
	*h.reported = (*h.reported)[:0]
	...
	if internal.MeshRefIndex(kept) != 1 {
		t.Fatalf("the kept ref has id %d, so the later frame's mesh did not reuse its slot", internal.MeshRefIndex(kept))
	}
	if pass := h.passes()[0]; pass.Instances != 0 {
...
func TestTopologyPassesThroughAndOnlyTriangleListsDivideByThree(t *testing.T) {
	...
	h.lookup(func(la scene.LookupAccess) {
		var record internal.MeshRecord
		record = internal.LookupMeshes(internal.LookupAccessLookup(la))[internal.MeshRefIndex(ref)-1]
