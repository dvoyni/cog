...
func TestPackInstanceWritesTheWorldMatrixAsThreeRows(t *testing.T) {
	...
	instance := packInstance(transform.Mat4(), internal.AnimBinding{Offset: internal.SceneNoAnim}, 0)
	want := [3]m.Vec4{
		{X: 1, W: 1},
		{Y: 1, W: 2},
		{Z: 1, W: 3},
...
func TestPackInstanceSkipsAnimationAndSkinningForABufferBuiltDraw(t *testing.T) {
	instance := packInstance(m.Mat4{}, internal.AnimBinding{Offset: internal.SceneNoAnim}, 0)
	if instance.AnimOffset != internal.SceneNoAnim {
		t.Fatalf("animOffset is %d, want SCENE_NO_ANIM (%d)", instance.AnimOffset, internal.SceneNoAnim)
	}
...
func TestPackInstanceFlagsNonUniformScaleOnly(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			instance := packInstance(test.matrix, internal.AnimBinding{Offset: internal.SceneNoAnim}, 0)
...
func TestPbrRecordMatchesItsShaderSideOffsets(t *testing.T) {
	if size := unsafe.Sizeof(internal.ScenePbrRecord{}); size != 160 {
		t.Fatalf("scenePbrRecord is %d bytes, want 160", size)
	}
	var record internal.ScenePbrRecord
	for _, test := range []struct {
		name   string
		offset uintptr
		want   uintptr
...
func TestAZeroIntensityMeansOne(t *testing.T) {
	block := packFrameLighting(sceneFrameBlock{}, scene.CameraDescr{
...
func TestPackInstanceRowsTransformLikeTheMatrix(t *testing.T) {
	...
	instance := packInstance(world, internal.AnimBinding{Offset: internal.SceneNoAnim}, 0)
