...
func TestASkinnedModelBindsItsOwnPosesAndSkins(t *testing.T) {
	...
	if got := len(boundBytes(t, h, "sceneSkinJoints")); got != internal.SkinJointSize {
		t.Errorf("the joint buffer is %d bytes, want the model's one joint at %d", got, internal.SkinJointSize)
	}
	instance := firstInstance(t, h)
	if instance.Flags&sceneNoSkin != 0 {
...
func TestTheLookupReportsClipsJointsAndPoseBytes(t *testing.T) {
	...
	if want := 62 * internal.PoseSize; bytes != want {
