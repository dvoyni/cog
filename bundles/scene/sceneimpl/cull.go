...
// error.
func resolveBounds(neverCull bool, explicit m.Sphere, mesh internal.MeshRecord) (m.Sphere, bool) {
	switch {
	case neverCull:
...
func prepareDraw(record internal.DrawRecord, mesh internal.MeshRecord) preparedDraw {
	world := record.World()
...
func (c *culler) cull(
	...
	draws []internal.DrawRecord, prepared []preparedDraw,
) int {
	...
	for i := range draws {
		if !internal.LayerMaskDrawnBy(draws[i].Layers, cullMask) {
			continue
		}
		result.recorded++
