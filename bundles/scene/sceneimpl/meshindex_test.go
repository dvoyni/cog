...
func indexWidthOf(t testing.TB, h *harness, ref scene.MeshRef) (gpu.IndexWidth, int) {
	...
	h.kernel.ExecuteCommand[lookupProbeCmd](lookupProbeRequest{lookup: func(lookup *scene.Lookup) {
		record, ok := internal.LookupMesh(lookup, ref)
		found = ok
...
func TestAModelPrimitiveNarrowsItsIndices(t *testing.T) {
	...
	h.kernel.ExecuteCommand[lookupProbeCmd](lookupProbeRequest{lookup: func(lookup *scene.Lookup) {
		for i := range internal.LookupMeshes(lookup) {
			if internal.LookupMeshes(lookup)[i].Indexed {
				widths = append(widths, internal.LookupMeshes(lookup)[i].IndexWidth)
				sizes = append(sizes, internal.LookupMeshes(lookup)[i].Indices.Size())
			}
		}
	}})
	if len(widths) != 1 {
...
func TestAUnitMeshNarrowsItsIndices(t *testing.T) {
	...
	h.kernel.ExecuteCommand[lookupProbeCmd](lookupProbeRequest{lookup: func(lookup *scene.Lookup) {
		for i := range internal.LookupMeshes(lookup) {
			if internal.LookupMeshes(lookup)[i].Indexed {
				width, size = internal.LookupMeshes(lookup)[i].IndexWidth, internal.LookupMeshes(lookup)[i].Indices.Size()
				found++
			}
		}
	}})
