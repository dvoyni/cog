...
func TestAPassNamingItsOwnDepthTextureKeepsIt(t *testing.T) {
	...
	h := newHarness(t, func(q *scene.OpQueue) {
		...
		descr.Passes = []scene.Pass{{
			...
			Depth: gfx.DepthTarget(sizedTexture(1024, 1024)), ClearDepth: m.Some(internal.DepthClearFar),
		}}
...
func TestColourIsAlwaysKeptAndAutomaticDepthDiscarded(t *testing.T) {
	...
	h := newHarness(t, func(q *scene.OpQueue) {
		descr := simpleCamera()
		descr.Passes = []scene.Pass{
			{Tag: "offscreen", Target: gfx.TextureTarget(sizedTexture(256, 256), 0, 0), ClearDepth: m.Some(internal.DepthClearFar)},
			{ClearDepth: m.Some(internal.DepthClearFar)},
		}
		q.Camera(cameraMain, descr)
	})
	h.frame()
...
func TestPassLabelsNameTheCameraAndTag(t *testing.T) {
	...
	h := newHarness(t, func(q *scene.OpQueue) {
		descr := simpleCamera()
		descr.Passes = []scene.Pass{
			{Tag: "shadow", Order: -1000, Target: gfx.NoTarget(), Depth: gfx.DepthTarget(sizedTexture(1024, 1024)), ClearDepth: m.Some(internal.DepthClearFar)},
			{ClearDepth: m.Some(internal.DepthClearFar)},
		}
		q.Camera(id, descr)
	})
