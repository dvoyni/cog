...
func BenchmarkCanvasFlushBoardText(b *testing.B) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		b.Fatalf("read font fixture: %v", err)
	}
	config := Config{AtlasSize: 256, LayersPerArray: 4, MaxAtlasBytes: 256 * 256 * 4 * 4}
	white := m.Color{R: 1, G: 1, B: 1, A: 1}
	k, _, backend := testKernel(b, fstest.MapFS{"font.ttf": &fstest.MapFile{Data: fontData}}, config, func(write *OpQueue) {
		for i := 0; i < 40; i++ {
			write.Text(Layer(i%4), "font.ttf", "HP 12/20", TextDraw{
				Position: m.Vec2{X: float32(i * 7), Y: float32(i * 5)}, Size: 16, Color: white,
			})
		}
	})
	runFrame(k)
...
func BenchmarkCanvasFlushCachedText(b *testing.B) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		b.Fatalf("read font fixture: %v", err)
	}
	...
	k, _, backend := testKernel(b, fstest.MapFS{"font.ttf": &fstest.MapFile{Data: fontData}}, config, func(write *OpQueue) {
		write.Text(0, "font.ttf", "Canvas cached text", TextDraw{
			Position: m.Vec2{X: 10, Y: 10}, Size: 18, Color: m.Color{R: 1, G: 1, B: 1, A: 1},
		})
	})
	runFrame(k)
	backend.capture = false
...
func TestMeasureTextSharesParsedFontAcrossSizes(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	filesystem := &testFS{FS: fstest.MapFS{"font.ttf": &fstest.MapFile{Data: fontData}}}
	config := Config{AtlasSize: 64, LayersPerArray: 2, MaxAtlasBytes: 64 * 64 * 4 * 2}
	k, _, _ := testKernel(t, filesystem, config, func(*OpQueue) {})
	var first, second m.Vec2
	probeLookup(k, func(la LookupAccess) {
		first = la.MeasureTextSize("font.ttf", 18, "AB")
		second = la.MeasureTextSize("font.ttf", 24, "A\nB")
	})
	if first.X <= 0 || first.Y <= 0 || second.X <= 0 || second.Y <= first.Y {
		t.Fatalf("measurements = first %+v second %+v", first, second)
	}
	if filesystem.opens != 1 {
		t.Fatalf("font opens = %d, want one parsed source shared across sizes", filesystem.opens)
	}
}

func TestTextRasterizesAtFramebufferScale(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	glyphWidth := func(framebuffer float32) int {
		filesystem := &testFS{FS: fstest.MapFS{"font.ttf": &fstest.MapFile{Data: fontData}}}
		config := Config{AtlasSize: 128, LayersPerArray: 2, MaxAtlasBytes: 128 * 128 * 4 * 2}
		k, _, backend := testKernel(t, filesystem, config, func(write *OpQueue) {
			write.Text(0, "font.ttf", "A", TextDraw{Position: m.Vec2{X: 10, Y: 20}, Size: 18, Color: m.Color{R: 1, G: 1, B: 1, A: 1}})
		})
		k.ExecuteCommand[app.SetViewportCmd](app.SetViewportRequest{
			Width: 100, Height: 100, FramebufferWidth: framebuffer, FramebufferHeight: framebuffer,
		})
		runFrame(k)
		widest := 0
		for _, update := range backend.updates {
			if update.region.Width > widest {
				widest = update.region.Width
			}
		}
		return widest
	}
	...
	twoX := glyphWidth(200)
	if oneX <= 0 || twoX <= 0 {
		t.Fatalf("glyph uploads = 1x %d, 2x %d, want both positive", oneX, twoX)
	}
	if twoX < oneX*2-6 || twoX > oneX*2+2 {
		t.Fatalf("2x glyph width = %d, want ~2x of %d (physical-scale rasterization)", twoX, oneX)
	}
}

func TestGetFontMetricsReturnsFaceMetrics(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	...
	var m FontMetrics
	probeLookup(k, func(la LookupAccess) {
		m = la.FontMetrics("font.ttf", 20)
	})
	if m.Ascent <= 0 || m.Descent <= 0 || m.CapHeight <= 0 || m.XHeight <= 0 || m.LineHeight <= 0 {
		t.Fatalf("metrics = %+v, want positive values", m)
	}
	if m.XHeight > m.CapHeight || m.CapHeight > m.Ascent || m.LineHeight < m.Ascent {
		t.Fatalf("metrics ordering off: %+v", m)
	}
}
...
func TestSpritesAndGlyphsUseSeparateAtlases(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	filesystem := &testFS{FS: fstest.MapFS{
		"sprite.png": &fstest.MapFile{Data: pngBytes(t, 2, 2)},
		"font.ttf":   &fstest.MapFile{Data: fontData},
	}}
	config := Config{AtlasSize: 64, LayersPerArray: 2, MaxAtlasBytes: 64 * 64 * 4 * 2}
	k, _, backend := testKernel(t, filesystem, config, func(write *OpQueue) {
		write.Sprite(0, "sprite.png", SpriteTransform{Size: m.Vec2{X: 8, Y: 8}}, nil)
		write.Text(0, "font.ttf", "A", TextDraw{Position: m.Vec2{X: 10, Y: 10}, Size: 18, Color: m.Color{R: 1, G: 1, B: 1, A: 1}})
	})
	runFrame(k)
	if len(backend.allocations) != 2 {
		t.Fatalf("atlas allocations = %d, want separate sprite and font arrays", len(backend.allocations))
	}
	textures := map[gfx.TextureID]bool{}
	for _, update := range backend.updates {
		textures[update.id] = true
	}
	if len(textures) != 2 {
		t.Fatalf("distinct atlas textures = %d, want sprite and font atlases separated", len(textures))
	}
	if backend.draws < 2 {
		t.Fatalf("draws = %d, want sprite and glyph", backend.draws)
	}
}

func TestTextEmitsTextRunsAndIconSprites(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	filesystem := &testFS{FS: fstest.MapFS{
		"font.ttf": &fstest.MapFile{Data: fontData},
		"icon.png": &fstest.MapFile{Data: pngBytes(t, 8, 8)},
	}}
	config := Config{AtlasSize: 128, LayersPerArray: 2, MaxAtlasBytes: 128 * 128 * 4 * 2}
	k, _, backend := testKernel(t, filesystem, config, func(write *OpQueue) {
		write.Text(0, "font.ttf", "A${icon.png}B", TextDraw{Position: m.Vec2{X: 10, Y: 20}, Size: 18, Color: m.Color{R: 1, G: 1, B: 1, A: 1}})
	})
	runFrame(k)
	if len(backend.allocations) != 2 {
		t.Fatalf("atlas allocations = %d, want font (glyphs) and sprite (icon) atlases", len(backend.allocations))
	}
	if backend.draws < 3 {
		t.Fatalf("draws = %d, want glyph A + icon + glyph B", backend.draws)
	}
}

func TestMeasureTextCountsIconAndText(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	filesystem := &testFS{FS: fstest.MapFS{
		"font.ttf": &fstest.MapFile{Data: fontData},
		"icon.png": &fstest.MapFile{Data: pngBytes(t, 16, 8)},
	}}
	config := Config{AtlasSize: 128, LayersPerArray: 2, MaxAtlasBytes: 128 * 128 * 4 * 2}
	k, _, _ := testKernel(t, filesystem, config, func(*OpQueue) {})

	var metrics FontMetrics
	var iconOnly, plain m.Vec2
	probeLookup(k, func(la LookupAccess) {
		metrics = la.FontMetrics("font.ttf", 20)
		iconOnly = la.MeasureTextSize("font.ttf", 20, "${icon.png}")
		plain = la.MeasureTextSize("font.ttf", 20, "Score")
	})
	wantIcon := metrics.CapHeight * 16.0 / 8.0
	if iconOnly.X < wantIcon-0.5 || iconOnly.X > wantIcon+0.5 {
		t.Fatalf("icon-only width = %v, want cap height x aspect = %v", iconOnly.X, wantIcon)
	}
	if plain.X <= 0 {
		t.Fatalf("plain text width = %v, want positive", plain.X)
	}
}

func TestMeasureWrappedTextSizeUsesWidthAndSplitsLongWords(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	...
	config := Config{AtlasSize: 128, LayersPerArray: 2, MaxAtlasBytes: 128 * 128 * 4 * 2}
	k, _, _ := testKernel(t, filesystem, config, func(*OpQueue) {})
	var line, wrapped, split, iconWord, zeroWidth m.Vec2
	probeLookup(k, func(la LookupAccess) {
		line = la.MeasureTextSize("font.ttf", 20, "one two")
		wrapped = la.MeasureWrappedTextSize("font.ttf", 20, "one two three", line.X)
		split = la.MeasureWrappedTextSize("font.ttf", 20, "abcdefgh", la.MeasureTextSize("font.ttf", 20, "abcd").X)
		iconWord = la.MeasureWrappedTextSize("font.ttf", 20, "ab${icon.png}cd", la.MeasureTextSize("font.ttf", 20, "ab${icon.png}").X)
		zeroWidth = la.MeasureWrappedTextSize("font.ttf", 20, "one two three", 0)
	})
	if wrapped.Y < line.Y*2-0.5 || wrapped.Y > line.Y*2+0.5 {
		t.Fatalf("wrapped height = %v, want two lines of %v", wrapped.Y, line.Y)
	}
	if wrapped.X > line.X+0.5 {
		t.Fatalf("wrapped width = %v, want at most %v", wrapped.X, line.X)
	}
	if split.Y < line.Y*2-0.5 || split.Y > line.Y*2+0.5 {
		t.Fatalf("split word height = %v, want two lines of %v", split.Y, line.Y)
	}
	if iconWord.Y < line.Y*2-0.5 || iconWord.Y > line.Y*2+0.5 {
		t.Fatalf("icon word height = %v, want two lines of %v", iconWord.Y, line.Y)
	}
	if unwrapped := func() m.Vec2 {
		var size m.Vec2
		probeLookup(k, func(la LookupAccess) { size = la.MeasureTextSize("font.ttf", 20, "one two three") })
		return size
	}(); zeroWidth != unwrapped {
		t.Fatalf("zero-width wrapped size = %+v, want unwrapped %+v", zeroWidth, unwrapped)
	}
}

func TestFontAtlasInvalidatesOnFramebufferScaleChange(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	...
	runFrame(k)
	allocsBefore := len(backend.allocations)
	releasesBefore := len(backend.releasedTextures)

	k.ExecuteCommand[app.SetViewportCmd](app.SetViewportRequest{
		Width: 100, Height: 100, FramebufferWidth: 100, FramebufferHeight: 100,
	})
	runFrame(k)
	if len(backend.releasedTextures) <= releasesBefore {
		t.Fatal("font atlas was not released when the framebuffer scale changed")
	}
	if len(backend.allocations) <= allocsBefore {
		t.Fatal("font atlas was not re-rasterized at the new scale")
	}
}
...
func TestMeasureTextSizeHandlesMultilineIconsAndEscaping(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	...
	config := Config{AtlasSize: 128, LayersPerArray: 2, MaxAtlasBytes: 128 * 128 * 4 * 2}
	k, _, _ := testKernel(t, filesystem, config, func(*OpQueue) {})
	var single, twoLine, escaped, literalIcon m.Vec2
	probeLookup(k, func(la LookupAccess) {
		single = la.MeasureTextSize("font.ttf", 20, "A${icon.png}B")
		twoLine = la.MeasureTextSize("font.ttf", 20, "A${icon.png}B\nA${icon.png}B")
		// A literal "${icon.png}" via escaping must measure wider than the icon form.
		escaped = la.MeasureTextSize("font.ttf", 20, "\\${icon.png}")
		literalIcon = la.MeasureTextSize("font.ttf", 20, "${icon.png}")
	})
	if twoLine.Y < single.Y*2-0.5 || twoLine.Y > single.Y*2+0.5 {
		t.Fatalf("two-line height = %v, want ~2x single %v", twoLine.Y, single.Y)
	}
	if twoLine.X < single.X-0.5 || twoLine.X > single.X+0.5 {
		t.Fatalf("two-line width = %v, want widest line %v", twoLine.X, single.X)
	}
	if escaped.X <= literalIcon.X {
		t.Fatalf("escaped literal width = %v, want wider than icon form %v", escaped.X, literalIcon.X)
	}
}

func TestUnloadFontDropsParsedSourceAndKeepsGlyphAtlas(t *testing.T) {
	fontData, err := os.ReadFile("../testdata/fonts/Gabriela-Regular.ttf")
	if err != nil {
		t.Fatalf("read font fixture: %v", err)
	}
	...
	runFrame(k)
	if filesystem.opens != 1 {
		t.Fatalf("font opens = %d, want one parsed source", filesystem.opens)
	}
	releasesBefore := len(backend.releasedTextures)

	probeLookup(k, func(la LookupAccess) { la.UnloadFont("font.ttf") })
	runFrame(k)

	// Only whole-atlas invalidation on resize frees glyph pages, so unloading a
	// font must not release its glyph atlas array.
	if len(backend.releasedTextures) != releasesBefore {
		t.Fatalf("released textures = %d, want unchanged %d (no per-font glyph free)", len(backend.releasedTextures), releasesBefore)
	}
	// The parsed source was dropped, so drawing re-reads the font file.
	if filesystem.opens != 2 {
		t.Fatalf("font opens after unload = %d, want re-read of the parsed source", filesystem.opens)
	}
}
