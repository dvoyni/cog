...
- The move is staged. `kernel/archtest` holds `aliasIndexRoots`, the plugins moved so far, and checks them under these rules and every other plugin under ADR 0002's. ecsaudio, ecsscene and ecsphysics2d moved first, and every other plugin but mcp after them, each `internal/types` merged into its `internal/`. mcp stays on ADR 0002 for now: its root is imported by every plugin that declares an MCP tool, and its `internal/` carries the MCP SDK, so a root importing it would put the SDK into all of them. When mcp moves, the ADR 0002 root rules and the list go.
