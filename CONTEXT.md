...
**Hook**:
One record of one act on an Entity, as seen through a Query: the act's kinds, and the values of the Query's fields at that moment, the last ones before an exit included. There are five kinds — spawned, despawned, entered the match, exited it, a value the Query names changed — and one act can be several at once, since a Spawn of a matching Entity is also an entry and a Despawn also an exit. A System asks for the kinds it wants and reads the Hooks for a Query in the order they happened, each System its own copy, gathered since the end of its own last run. A Structural change and a change of value are recorded in the same form and read the same way, and nothing runs on a Hook's behalf at the moment of change.
