...
const timeDescription = "Stop, start or single-step the game's update loop. `pause` stops update " +
	...
	"no time is banked and nothing catches up. `status` just reports.\n\n" +
	...
	"each need a tick, so while paused they perform one step themselves and say so; arming them " +
	"together shares a single step, and `gfx_capture` should come last because it costs no " +
	"tick.\n\n" +
	...
	"you disconnect — it stays paused until something resumes it."

// TimeRequest asks the engine to pause, resume, step or report.
type TimeRequest struct {
	Action string `json:"action" jsonschema:"one of pause, resume, step or status"`
...
func timeControl(k kernel.Executioner, request TimeRequest) (TimeResponse, error) {
	...
	if !known {
		return TimeResponse{}, mcp.Unavailable{Reason: fmt.Sprintf(
			"%q is not an action — use pause, resume, step or status", request.Action)}
	}
	...
	if action == app.TimeStep {
		ctx, cancel := context.WithTimeout(k.Context(), stepDeadline)
		defer cancel()
		k = k.WithContext(ctx)
	}
	answer, err := k.ExecuteCommand[app.TimeCmd](app.TimeRequest{Action: action, Steps: request.Steps})
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return TimeResponse{}, mcp.Unavailable{Reason: fmt.Sprintf(
				"no tick was published within %s — the window may be minimised or the game may "+
					"have stopped rendering; the steps will run when it draws again",
				stepDeadline)}
		}
